...
#if ARDUINO >= 100
#else
void Print::write(const char *str) { write((const uint8_t *)str, strlen(str)); }
#endif
...
// Adapted from Hacker's Delight (Henry Warren, ISBN 0321842685)
// www.hackersdelight.org by Rob Tillaart, Tom Carpenter, "genom2" with input
// from others... http://forum.arduino.cc/index.php?topic=167414.0
//
...
#endif
{
  ...
  while (n & 0xff000000) {
    divmod10_asm32(n, digit, tmp8);
    *--p = digit + '0';
  }
  while (n & 0xff0000) {
    divmod10_asm24(n, digit, tmp8);
    *--p = digit + '0';
  }
  while (n & 0xff00) {
    divmod10_asm16(n, digit, tmp8);
    *--p = digit + '0';
  }
  while ((n & 0xff) > 9) {
    divmod10_asm8(n, digit, tmp8);
    *--p = digit + '0';
  }
  *--p = n + '0';
