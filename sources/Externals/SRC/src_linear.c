...
static SRC_STATE_VT linear_state_vt = {linear_vari_process, linear_vari_process,
                                       linear_reset, linear_close};

static SRC_STATE static_linear_state;
static LINEAR_DATA static_linear_data;
static float static_linear_last_value[MAX_CHANNELS];
static int static_linear_in_use = 0;
...
static SRC_ERROR linear_vari_process(SRC_STATE *state, SRC_DATA *data) {
  ...
  while (input_index < 1.0 && priv->out_gen < priv->out_count) {
    ...
    if (priv->out_count > 0 &&
      ...
      src_ratio = state->last_ratio +
                  priv->out_gen * (data->src_ratio - state->last_ratio) /
                      priv->out_count;

    for (ch = 0; ch < state->channels; ch++) {
      data->data_out[priv->out_gen] =
          (float)(priv->last_value[ch] +
                  input_index *
  ...
  while (priv->out_gen < priv->out_count &&
    ...
    for (ch = 0; ch < state->channels; ch++) {
      data->data_out[priv->out_gen] =
          (float)(data->data_in[priv->in_used - state->channels + ch] +
                  input_index *
                      ((double)data->data_in[priv->in_used + ch] -
                       data->data_in[priv->in_used - state->channels + ch]));
      priv->out_gen++;
    };
...
LIBSAMPLERATE_DLL_PRIVATE const char *linear_get_name(int src_enum) {
...
LIBSAMPLERATE_DLL_PRIVATE const char *linear_get_description(int src_enum) {
  if (src_enum == SRC_LINEAR)
...
static LINEAR_DATA *linear_data_new(int channels) {
  assert(channels > 0);

  LINEAR_DATA *priv = &static_linear_data;
  memset(priv, 0, sizeof(LINEAR_DATA));
  priv->linear_magic_marker = LINEAR_MAGIC_MARKER;

  if (channels > MAX_CHANNELS)
    return NULL;

  priv->last_value = static_linear_last_value;

  return priv;
}

LIBSAMPLERATE_DLL_PRIVATE SRC_STATE *linear_state_new(int channels,
                                                      SRC_ERROR *error) {
  assert(channels > 0);
  assert(error != NULL);
  ...
  if (static_linear_in_use) {
    *error = SRC_ERR_BAD_STATE;
    return NULL;
  }

  SRC_STATE *state = &static_linear_state;
