  ...
  if (data->data_in < data->data_out) {
    if (data->data_in + data->input_frames * state->channels >
        data->data_out) { /*-printf ("\n\ndata_in: %p    data_out: %p\n",
                                  (void*) (data->data_in + data->input_frames *
                             psrc->channels), (void*) data->data_out) ;-*/
      return SRC_ERR_DATA_OVERLAP;
    };
  } else if (data->data_out + data->output_frames * state->channels >
             data->data_in) { /*-printf ("\n\ndata_in : %p   ouput frames: %ld
                              data_out: %p\n", (void*) data->data_in,
                              data->output_frames, (void*) data->data_out) ;

                              printf ("data_out: %p (%p)    data_in: %p\n",
                              (void*) data->data_out, (void*) (data->data_out +
                              data->input_frames * psrc->channels), (void*)
                              data->data_in) ;-*/
    return SRC_ERR_DATA_OVERLAP;
  };
...
long src_callback_read(SRC_STATE *state, double src_ratio, long frames,
  ...
  output_frames_gen = 0;
  while (output_frames_gen <
         frames) { /*	Use a dummy array for the case where the callback
                    *function *	returns without setting the ptr.
                    */
    float dummy[1];
    ...
    if (src_data.input_frames == 0) {
      ...
      src_data.input_frames =
          state->callback_func(state->user_callback_data, &ptr);
...
const char *src_strerror(int error) {
  ...
  case SRC_ERR_BAD_SRC_RATIO:
    return "SRC ratio outside [1/" SRC_MAX_RATIO_STR ", " SRC_MAX_RATIO_STR
           "] range.";
  ...
  case SRC_ERR_SIZE_INCOMPATIBILITY:
    return "Internal error. Input data / internal buffer size difference. "
           "Please report this.";
  ...
  case SRC_ERR_BAD_MODE:
    return "Calling mode differs from initialisation mode (ie process v "
           "callback).";
