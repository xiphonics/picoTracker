...
static SRC_STATE_VT sinc_stereo_state_vt = {
    sinc_stereo_vari_process, sinc_stereo_vari_process, sinc_reset, sinc_close};

static SRC_STATE_VT sinc_mono_state_vt = {
    sinc_mono_vari_process, sinc_mono_vari_process, sinc_reset, sinc_close};

#ifdef ENABLE_SINC_MEDIUM_CONVERTER
#define SINC_STATIC_MAX_HALF_LEN 22437
#define SINC_STATIC_MAX_INDEX_INC 491
#else
#define SINC_STATIC_MAX_HALF_LEN 2463
#define SINC_STATIC_MAX_INDEX_INC 128
#endif

#define SINC_STATIC_BASE_B_LEN_TARGET                                          \
  (3 * (((((SINC_STATIC_MAX_HALF_LEN + 2) * SRC_MAX_RATIO) +                   \
          (SINC_STATIC_MAX_INDEX_INC / 2)) /                                   \
         SINC_STATIC_MAX_INDEX_INC) +                                          \
        1))
#define SINC_STATIC_BASE_B_LEN                                                 \
  ((SINC_STATIC_BASE_B_LEN_TARGET > 4096) ? SINC_STATIC_BASE_B_LEN_TARGET      \
                                          : 4096)
#define SINC_STATIC_BUFFER_LEN                                                 \
  (MAX_CHANNELS * SINC_STATIC_BASE_B_LEN + MAX_CHANNELS + 1)

/* Static storage backing the single allowed sinc instance. */
#ifdef ADV
static SRC_STATE static_sinc_state;
static SINC_FILTER static_sinc_filter;
__attribute__((
    aligned(32))) static float static_sinc_buffer[SINC_STATIC_BUFFER_LEN];
#endif
static int static_sinc_in_use = 0;
...
static inline int
int_div_ceil(int divident,
             int divisor) /* == (int) ceil ((float) divident / divisor) */
{
...
LIBSAMPLERATE_DLL_PRIVATE const char *sinc_get_name(int src_enum) {
  switch (src_enum) {
...
LIBSAMPLERATE_DLL_PRIVATE const char *sinc_get_description(int src_enum) {
  switch (src_enum) {
  case SRC_SINC_FASTEST:
    return "Band limited sinc interpolation, fastest, 97dB SNR, 80% BW.";

  case SRC_SINC_MEDIUM_QUALITY:
    return "Band limited sinc interpolation, medium quality, 121dB SNR, 90% "
           "BW.";
...
static SINC_FILTER *sinc_filter_new(int converter_type, int channels) {
  ...
  if (static_sinc_in_use)
    return NULL;

  SINC_FILTER *priv = &static_sinc_filter;
  memset(priv, 0, sizeof(SINC_FILTER));
  ...
  priv->b_len = 3 * (int)psf_lrint((priv->coeff_half_len + 2.0) /
                                       priv->index_inc * SRC_MAX_RATIO +
                                   1);
  priv->b_len = MAX(priv->b_len, 4096);
  priv->b_len *= channels;
  priv->b_len += 1; // There is a <= check against samples_in_hand requiring a
                    // buffer bigger than the calculation above

  if (priv->b_len + channels > SINC_STATIC_BUFFER_LEN) {
    return NULL;
  }

  priv->buffer = static_sinc_buffer;
...
sinc_state_new(int converter_type, int channels, SRC_ERROR *error) {
  ...
  if (static_sinc_in_use) {
    *error = SRC_ERR_BAD_SINC_STATE;
    return NULL;
  }

  SRC_STATE *state = &static_sinc_state;
  ...
  if (!state->private_data) {
    *error = SRC_ERR_SINC_BAD_BUFFER_LEN;
    return NULL;
  }

  static_sinc_in_use = 1;
...
static void sinc_reset(SRC_STATE *state) {
  ...
  memset(filter->buffer + filter->b_len, 0xAA,
         state->channels * sizeof(filter->buffer[0]));
} /* sinc_reset */
...
static inline double calc_output_single(SINC_FILTER *filter,
  ...
  if (data_index < 0) /* Avoid underflow access to filter->buffer. */
  {
    ...
    /* If the assert triggers we would have to take care not to
     * underflow/overflow */
  ...
  while (filter_index >= MAKE_INCREMENT_T(0)) {
    ...
    icoeff = filter->coeffs[indx] +
             fraction * (filter->coeffs[indx + 1] - filter->coeffs[indx]);
...
static SRC_ERROR sinc_mono_vari_process(SRC_STATE *state, SRC_DATA *data) {
  ...
  filter->b_current =
      (filter->b_current + state->channels * psf_lrint(input_index - rem)) %
      filter->b_len;
  input_index = rem;
  ...
  while (filter->out_gen < filter->out_count) {
    ...
    samples_in_hand =
        (filter->b_end - filter->b_current + filter->b_len) % filter->b_len;
    ...
    if (filter->out_count > 0 &&
      ...
      src_ratio = state->last_ratio +
                  filter->out_gen * (data->src_ratio - state->last_ratio) /
                      filter->out_count;
    ...
    data->data_out[filter->out_gen] =
        (float)((float_increment / filter->index_inc) *
...
static inline void calc_output_stereo(SINC_FILTER *filter, int channels,
                                      increment_t increment,
                                      increment_t start_filter_index,
                                      double scale, float *output) {
...
static inline void calc_output_quad(SINC_FILTER *filter, int channels,
                                    increment_t increment,
                                    increment_t start_filter_index,
                                    double scale, float *output) {
...
static inline void calc_output_hex(SINC_FILTER *filter, int channels,
                                   increment_t increment,
                                   increment_t start_filter_index, double scale,
                                   float *output) {
...
static inline void calc_output_multi(SINC_FILTER *filter, increment_t increment,
                                     increment_t start_filter_index,
                                     int channels, double scale,
                                     float *output) {
  double fraction, icoeff;
  /* The following line is 1999 ISO Standard C. If your compiler complains, get
   * a better compiler. */
  double *left, *right;
  increment_t filter_index, max_filter_index;
  int data_index, coeff_count, indx;
...
static SRC_ERROR prepare_data(SINC_FILTER *filter, int channels, SRC_DATA *data,
  ...
  if (filter->b_current ==
      0) { /* Initial state. Set up zeros at the start of the buffer and
  ...
  if (filter->in_used == filter->in_count &&
      ...
      data->end_of_input) { /* Handle the case where all data in the current
                             *buffer has been
                             ** consumed and this is the last buffer.
                             */

    if (filter->b_len - filter->b_end <
