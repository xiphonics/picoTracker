...
public:
  ...
  // TODO (democloid): original code called "EnsureCapacity" in order to grow
  // the _stack or _buffer using dynamic allocations. We removed this and
  // replaced with an assert, as our use case does not go over those limits. we
  // should probably have a better check here and propagate errors down, but
  // right now the limitation is XML nesting depth, which is 3 at the time of
