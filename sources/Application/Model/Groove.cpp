...
unsigned char GrooveBase::data_[MAX_GROOVES][16];

GrooveBase::GrooveBase() : Persistent("GROOVES") { Clear(); };

void GrooveBase::Clear() {
...
void GrooveBase::GetChannelData(int channel, int *groove, int *position) {
  ChannelGroove &c = channelGroove_[channel];
  *groove = c.groove_;
  *position = c.position_;
};

void GrooveBase::SaveContent(tinyxml2::XMLPrinter *printer) {
  saveHexBuffer(printer, "DATA", (unsigned char *)data_, 16 * MAX_GROOVES);
};

void GrooveBase::RestoreContent(PersistencyDocument *doc) {
  if (doc->FirstChild()) {
...
bool GrooveBase::UpdateGroove(ChannelGroove &c, bool reverse) {
...
void GrooveBase::SetGroove(int channel, int groove) {
...
bool GrooveBase::TriggerChannel(int i) {
  ChannelGroove &c = channelGroove_[i];
  return ((c.ticks_) % (data_[c.groove_][c.position_]) == 0);
};

unsigned char *GrooveBase::GetGrooveData(int groove) { return data_[groove]; };
