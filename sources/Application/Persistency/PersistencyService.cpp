...
PersistencyResult PersistencyService::ExportTheme(Theme *theme,
                                                  etl::string<32> name,
                                                  bool overwrite) {
  auto fs = FileSystem::GetInstance();

  // Add .ptt extension to the filename
  etl::string<64> filename = name;
  filename.append(THEME_FILE_EXTENSION);

  // Create themes directory if it doesn't exist
  if (!fs->exists(THEMES_DIR)) {
    if (!fs->makeDir(THEMES_DIR, false)) {
      Trace::Error("PERSISTENCYSERVICE: Could not create themes directory");
      return PERSIST_ERROR;
    }
  }

  etl::vector<const char *, 2> segments = {THEMES_DIR, filename.c_str()};
  CreatePath(pathBufferA, segments);

  // check if file already exists
  if (fs->exists(pathBufferA.c_str()) && !overwrite) {
    Trace::Log("PERSISTENCYSERVICE", "Theme file already exists: %s",
               pathBufferA.c_str());
    return PERSIST_EXISTS;
  }
  ...
  // Set the theme name
  theme->SetName(name.c_str());

  // Save current config settings to the theme
  theme->SaveFromConfig();

  tinyxml2::XMLPrinter printer(fp);

  // Use the theme's Persistent interface to save its data
  theme->Save(&printer);

  fp->Close();
  Trace::Log("PERSISTENCYSERVICE", "Successfully exported theme to: %s",
             pathBufferA.c_str());
  return PERSIST_SAVED;
}

PersistencyResult PersistencyService::ImportTheme(Theme *theme,
                                                  const char *name) {
  auto fs = FileSystem::GetInstance();

  if (!fs->chdir(THEMES_DIR)) {
    Trace::Error("PERSISTENCYSERVICE: Could not change to themes directory");
    return PERSIST_ERROR;
  }
  ...
  // Find the THEME element
  bool elem = doc.FirstChild();
  if (!elem || strcmp(doc.ElemName(), "THEME")) {
    Trace::Error("PERSISTENCYSERVICE: Could not find THEME node in file: %s",
                 name);
    return PERSIST_ERROR;
  }

  // Restore the theme content
  if (!theme->Restore(&doc)) {
    Trace::Error("PERSISTENCYSERVICE: Failed to restore theme from file: %s",
                 name);
    return PERSIST_ERROR;
  }

  // Extract theme name from filename (minus .ptt extension)
  etl::string<32> themeName;
  const char *dotPos = strrchr(name, '.');
  if (dotPos) {
    // Calculate the length of the name without extension
    size_t nameLength = dotPos - name;
    // Copy only up to 32 characters
    nameLength = nameLength < 32 ? nameLength : 31;
    themeName.assign(name, nameLength);
  } else {
    // No extension found, use the whole name (up to 32 characters)
    themeName.assign(name, strlen(name) < 32 ? strlen(name) : 31);
  }

  // Set the theme name if it wasn't already set in the XML
  if (strcmp(theme->GetName(), "Imported Theme") == 0) {
    theme->SetName(themeName.c_str());
  }

  // Apply the theme settings to the config
  theme->ApplyToConfig();

  Trace::Log("PERSISTENCYSERVICE", "Successfully imported theme settings");
  Trace::Log("PERSISTENCYSERVICE", "Theme name: %s", theme->GetName());
  return PERSIST_LOADED;
}
