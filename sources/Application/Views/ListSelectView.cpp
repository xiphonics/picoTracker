...
ListSelectView::ListSelectView(GUIWindow &w, ViewData *viewData)
    : FieldView(w, viewData) {}

void ListSelectView::OnFocus(){};

void ListSelectView::OnPlayerUpdate(PlayerEventType eventType,
                                    unsigned int tick) {}
...
void ListSelectView::SetContent(T_SimpleList<Path> &content) {
  ...
  for (it->Begin(); !it->IsDone(); it->Next()) {
    ...
    UIActionField *a1 =
        new UIActionField(current.GetPath(), LIST_SELECT, position);
    a1->AddObserver(*this);
