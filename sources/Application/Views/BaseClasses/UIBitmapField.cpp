...
UIBitmapField::UIBitmapField(GUIPoint &position, uint8_t width, uint8_t height, 
                             const uint8_t *bitmap_data, uint16_t fg_color, uint16_t bg_color)
    : UIField(position), width_(width), height_(height), bitmap_data_(bitmap_data),
...
void UIBitmapField::Draw(GUIWindow &w, int offset) {
  ...
  chargfx_draw_bitmap(x_, y_, width_, height_, bitmap_data_, fg_color_, bg_color_);
}
