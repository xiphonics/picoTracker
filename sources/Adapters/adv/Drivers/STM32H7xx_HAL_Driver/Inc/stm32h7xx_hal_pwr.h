...
#define PWR_PVDLEVEL_0                                                         \
  PWR_CR1_PLS_LEV0 /*!< Programmable voltage detector                          \
                        level 0 selection : 1V95       */
#define PWR_PVDLEVEL_1                                                         \
  PWR_CR1_PLS_LEV1 /*!< Programmable voltage detector                          \
                        level 1 selection : 2V1        */
#define PWR_PVDLEVEL_2                                                         \
  PWR_CR1_PLS_LEV2 /*!< Programmable voltage detector                          \
                        level 2 selection : 2V25       */
#define PWR_PVDLEVEL_3                                                         \
  PWR_CR1_PLS_LEV3 /*!< Programmable voltage detector                          \
                        level 3 selection : 2V4        */
#define PWR_PVDLEVEL_4                                                         \
  PWR_CR1_PLS_LEV4 /*!< Programmable voltage detector                          \
                        level 4 selection : 2V55       */
#define PWR_PVDLEVEL_5                                                         \
  PWR_CR1_PLS_LEV5 /*!< Programmable voltage detector                          \
                        level 5 selection : 2V7        */
#define PWR_PVDLEVEL_6                                                         \
  PWR_CR1_PLS_LEV6 /*!< Programmable voltage detector                          \
...
#define PWR_PVD_MODE_IT_RISING                                                 \
  (0x00010001U) /*!< Interrupt Mode with Rising edge trigger detection */
#define PWR_PVD_MODE_IT_FALLING                                                \
  (0x00010002U) /*!< Interrupt Mode with Falling edge trigger detection */
#define PWR_PVD_MODE_IT_RISING_FALLING                                         \
  (0x00010003U) /*!< Interrupt Mode with Rising/Falling edge trigger detection \
                 */
...
#define __HAL_PWR_GET_FLAG(__FLAG__)                                           \
  (((__FLAG__) == PWR_FLAG_PVDO)                                               \
       ? ((PWR->CSR1 & PWR_CSR1_PVDO) == PWR_CSR1_PVDO)                        \
   : ((__FLAG__) == PWR_FLAG_AVDO)                                             \
       ? ((PWR->CSR1 & PWR_CSR1_AVDO) == PWR_CSR1_AVDO)                        \
   : ((__FLAG__) == PWR_FLAG_ACTVOSRDY)                                        \
       ? ((PWR->CSR1 & PWR_CSR1_ACTVOSRDY) == PWR_CSR1_ACTVOSRDY)              \
   : ((__FLAG__) == PWR_FLAG_VOSRDY)                                           \
       ? ((PWR->D3CR & PWR_D3CR_VOSRDY) == PWR_D3CR_VOSRDY)                    \
   : ((__FLAG__) == PWR_FLAG_SMPSEXTRDY)                                       \
       ? ((PWR->CR3 & PWR_CR3_SMPSEXTRDY) == PWR_CR3_SMPSEXTRDY)               \
   : ((__FLAG__) == PWR_FLAG_BRR)                                              \
       ? ((PWR->CR2 & PWR_CR2_BRRDY) == PWR_CR2_BRRDY)                         \
   : ((__FLAG__) == PWR_FLAG_CPU_HOLD)                                         \
       ? ((PWR->CPU2CR & PWR_CPU2CR_HOLD1F) == PWR_CPU2CR_HOLD1F)              \
   : ((__FLAG__) == PWR_FLAG_CPU2_HOLD)                                        \
       ? ((PWR->CPUCR & PWR_CPUCR_HOLD2F) == PWR_CPUCR_HOLD2F)                 \
   : ((__FLAG__) == PWR_FLAG_SB)                                               \
       ? ((PWR->CPUCR & PWR_CPUCR_SBF) == PWR_CPUCR_SBF)                       \
   : ((__FLAG__) == PWR_FLAG2_SB)                                              \
       ? ((PWR->CPU2CR & PWR_CPU2CR_SBF) == PWR_CPU2CR_SBF)                    \
   : ((__FLAG__) == PWR_FLAG_STOP)                                             \
       ? ((PWR->CPUCR & PWR_CPUCR_STOPF) == PWR_CPUCR_STOPF)                   \
   : ((__FLAG__) == PWR_FLAG2_STOP)                                            \
       ? ((PWR->CPU2CR & PWR_CPU2CR_STOPF) == PWR_CPU2CR_STOPF)                \
   : ((__FLAG__) == PWR_FLAG_SB_D1)                                            \
       ? ((PWR->CPUCR & PWR_CPUCR_SBF_D1) == PWR_CPUCR_SBF_D1)                 \
   : ((__FLAG__) == PWR_FLAG2_SB_D1)                                           \
       ? ((PWR->CPU2CR & PWR_CPU2CR_SBF_D1) == PWR_CPU2CR_SBF_D1)              \
   : ((__FLAG__) == PWR_FLAG_SB_D2)                                            \
       ? ((PWR->CPUCR & PWR_CPUCR_SBF_D2) == PWR_CPUCR_SBF_D2)                 \
   : ((__FLAG__) == PWR_FLAG2_SB_D2)                                           \
       ? ((PWR->CPU2CR & PWR_CPU2CR_SBF_D2) == PWR_CPU2CR_SBF_D2)              \
   : ((__FLAG__) == PWR_FLAG_USB33RDY)                                         \
       ? ((PWR->CR3 & PWR_CR3_USB33RDY) == PWR_CR3_USB33RDY)                   \
   : ((__FLAG__) == PWR_FLAG_TEMPH)                                            \
       ? ((PWR->CR2 & PWR_CR2_TEMPH) == PWR_CR2_TEMPH)                         \
   : ((__FLAG__) == PWR_FLAG_TEMPL)                                            \
       ? ((PWR->CR2 & PWR_CR2_TEMPL) == PWR_CR2_TEMPL)                         \
   : ((__FLAG__) == PWR_FLAG_VBATH)                                            \
       ? ((PWR->CR2 & PWR_CR2_VBATH) == PWR_CR2_VBATH)                         \
       : ((PWR->CR2 & PWR_CR2_VBATL) == PWR_CR2_VBATL))
#else                         /* Single core lines */
#if defined(PWR_CPUCR_SBF_D2) /* STM32H72x, STM32H73x, STM32H74x and STM32H75x \
                                 lines */
...
#define __HAL_PWR_GET_FLAG(__FLAG__)                                           \
  (((__FLAG__) == PWR_FLAG_PVDO)                                               \
   ...
   : ((__FLAG__) == PWR_FLAG_VOSRDY)                                           \
       ? ((PWR->D3CR & PWR_D3CR_VOSRDY) == PWR_D3CR_VOSRDY)                    \
   : ((__FLAG__) == PWR_FLAG_SMPSEXTRDY)                                       \
       ? ((PWR->CR3 & PWR_FLAG_SMPSEXTRDY) == PWR_FLAG_SMPSEXTRDY)             \
   : ((__FLAG__) == PWR_FLAG_BRR)                                              \
       ? ((PWR->CR2 & PWR_CR2_BRRDY) == PWR_CR2_BRRDY)                         \
   : ((__FLAG__) == PWR_FLAG_SB)                                               \
       ? ((PWR->CPUCR & PWR_CPUCR_SBF) == PWR_CPUCR_SBF)                       \
   ...
   : ((__FLAG__) == PWR_FLAG_VBATH)                                            \
       ? ((PWR->CR2 & PWR_CR2_VBATH) == PWR_CR2_VBATH)                         \
       : ((PWR->CR2 & PWR_CR2_VBATL) == PWR_CR2_VBATL))
#else /* STM32H723, STM32H733, STM32H742, STM32H743, STM32H750 and STM32H753   \
         lines */
#define __HAL_PWR_GET_FLAG(__FLAG__)                                           \
  (((__FLAG__) == PWR_FLAG_PVDO)                                               \
       ? ((PWR->CSR1 & PWR_CSR1_PVDO) == PWR_CSR1_PVDO)                        \
   ...
   : ((__FLAG__) == PWR_FLAG_VOSRDY)                                           \
       ? ((PWR->D3CR & PWR_D3CR_VOSRDY) == PWR_D3CR_VOSRDY)                    \
   : ((__FLAG__) == PWR_FLAG_SCUEN)                                            \
       ? ((PWR->CR3 & PWR_CR3_SCUEN) == PWR_CR3_SCUEN)                         \
   : ((__FLAG__) == PWR_FLAG_BRR)                                              \
       ? ((PWR->CR2 & PWR_CR2_BRRDY) == PWR_CR2_BRRDY)                         \
   : ((__FLAG__) == PWR_FLAG_SB)                                               \
       ? ((PWR->CPUCR & PWR_CPUCR_SBF) == PWR_CPUCR_SBF)                       \
...
#define __HAL_PWR_GET_FLAG(__FLAG__)                                           \
  (((__FLAG__) == PWR_FLAG_PVDO)                                               \
   ...
   : ((__FLAG__) == PWR_FLAG_BRR)                                              \
       ? ((PWR->CR2 & PWR_CR2_BRRDY) == PWR_CR2_BRRDY)                         \
   : ((__FLAG__) == PWR_FLAG_VOSRDY)                                           \
       ? ((PWR->SRDCR & PWR_SRDCR_VOSRDY) == PWR_SRDCR_VOSRDY)                 \
   : ((__FLAG__) == PWR_FLAG_STOP)                                             \
       ? ((PWR->CPUCR & PWR_CPUCR_STOPF) == PWR_CPUCR_STOPF)                   \
   : ((__FLAG__) == PWR_FLAG_SB)                                               \
       ? ((PWR->CPUCR & PWR_CPUCR_SBF) == PWR_CPUCR_SBF)                       \
   : ((__FLAG__) == PWR_FLAG_MMCVDO)                                           \
       ? ((PWR->CSR1 & PWR_CSR1_MMCVDO) == PWR_CSR1_MMCVDO)                    \
   : ((__FLAG__) == PWR_FLAG_SMPSEXTRDY)                                       \
       ? ((PWR->CR3 & PWR_CR3_SMPSEXTRDY) == PWR_CR3_SMPSEXTRDY)               \
   : ((__FLAG__) == PWR_FLAG_USB33RDY)                                         \
       ? ((PWR->CR3 & PWR_CR3_USB33RDY) == PWR_CR3_USB33RDY)                   \
 ...
 * @retval None.
 */
#define __HAL_PWR_PVD_EXTID2_ENABLE_IT()                                       \
  SET_BIT(EXTI_D2->IMR1, PWR_EXTI_LINE_PVD)
#endif /* defined (DUAL_CORE) */
 ...
 * @retval None.
 */
#define __HAL_PWR_PVD_EXTID2_DISABLE_IT()                                      \
  CLEAR_BIT(EXTI_D2->IMR1, PWR_EXTI_LINE_PVD)
#endif /* defined (DUAL_CORE) */
 ...
 * @retval None.
 */
#define __HAL_PWR_PVD_EXTID2_ENABLE_EVENT()                                    \
  SET_BIT(EXTI_D2->EMR1, PWR_EXTI_LINE_PVD)
#endif /* defined (DUAL_CORE) */
 ...
 * @retval None.
 */
#define __HAL_PWR_PVD_EXTID2_DISABLE_EVENT()                                   \
  CLEAR_BIT(EXTI_D2->EMR1, PWR_EXTI_LINE_PVD)
#endif /* defined (DUAL_CORE) */
...
#define __HAL_PWR_PVD_EXTI_GET_FLAG()                                          \
  ((READ_BIT(EXTI->PR1, PWR_EXTI_LINE_PVD) == PWR_EXTI_LINE_PVD) ? 1UL : 0UL)
 ...
 */
#define __HAL_PWR_PVD_EXTID2_GET_FLAG()                                        \
  ((READ_BIT(EXTI_D2->PR1, PWR_EXTI_LINE_PVD) == PWR_EXTI_LINE_PVD) ? 1UL : 0UL)
#endif /* defined (DUAL_CORE) */
 ...
 * @retval None.
 */
#define __HAL_PWR_PVD_EXTID2_CLEAR_FLAG()                                      \
  SET_BIT(EXTI_D2->PR1, PWR_EXTI_LINE_PVD)
#endif /* defined (DUAL_CORE) */
