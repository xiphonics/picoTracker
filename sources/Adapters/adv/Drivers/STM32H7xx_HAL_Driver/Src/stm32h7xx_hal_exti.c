/**
  ...
  ******************************************************************************
  ...
  [..]
    ...
    (+) PendClearSource used to set the D3 Smart Run Domain autoamtic pend clear
  source. It is applicable for line with wkaeup target is Any (CPU1 , CPU2 and
  D3 smart run domain). Value can be one of the following:
        (++)  EXTI_D3_PENDCLR_SRC_NONE : no pend clear source is selected :
              In this case corresponding bit of D2PMRx register is set to 0
                (+++) On a configurable Line : the D3 domain wakeup signal is
                      automatically cleared after after the Delay + Rising Edge
  detect
  ...
  [..]
    ...
    (#) Configure the EXTI line using HAL_EXTI_SetConfigLine().
        ...
        (++) For Exti lines with wkaeup target is Any (CPU1 , CPU2 and D3 smart
  run domain), choose gpio D3 PendClearSource using PendClearSource member from
  EXTI_PendClear_Source structure.
...
HAL_StatusTypeDef HAL_EXTI_SetConfigLine(EXTI_HandleTypeDef *hexti,
  ...
  *regaddr = regval;
  ...
  /* The event mode cannot be configured if the line does not support it */
  assert_param(
      ((pExtiConfig->Line & EXTI_EVENT) == EXTI_EVENT) ||
  ...
  if ((pExtiConfig->Line & EXTI_TARGET_MASK) == EXTI_TARGET_MSK_ALL) {
    ...
    if (pExtiConfig->PendClearSource == EXTI_D3_PENDCLR_SRC_NONE) {
      ...
      *regaddr = regval;
      ...
      if (linepos < 16UL) {
        regaddr =
            (__IO uint32_t *)(&EXTI->D3PCR1L + (EXTI_CONFIG_OFFSET * offset));
        pcrlinepos = 1UL << linepos;
      } else {
        regaddr =
            (__IO uint32_t *)(&EXTI->D3PCR1H + (EXTI_CONFIG_OFFSET * offset));
        pcrlinepos = 1UL << (linepos - 16UL);
      }

      regval = (*regaddr & (~(pcrlinepos * pcrlinepos * 3UL))) |
               (pcrlinepos * pcrlinepos * (pExtiConfig->PendClearSource - 1UL));
      *regaddr = regval;
    }
  }
...
HAL_StatusTypeDef HAL_EXTI_GetConfigLine(EXTI_HandleTypeDef *hexti,
  ...
  if ((pExtiConfig->Line & EXTI_TARGET_MASK) == EXTI_TARGET_MSK_ALL) {
    ...
    if (((*regaddr) & linepos) != 0UL) {
      /* if wakeup target is any and PMR set, the read pend clear source from
       * D3PCRxL/H */
      if (linepos < 16UL) {
        regaddr =
            (__IO uint32_t *)(&EXTI->D3PCR1L + (EXTI_CONFIG_OFFSET * offset));
      ...
      pExtiConfig->PendClearSource =
          1UL + ((*regaddr & (pcrlinepos * pcrlinepos * 3UL)) /
                 (pcrlinepos * pcrlinepos));
    }
  }
  ...
  if ((hexti->Line & EXTI_CONFIG) != 0x00U) {
    ...
    *regaddr = regval;
    ...
    if ((hexti->Line & EXTI_GPIO) == EXTI_GPIO) {
      ...
      regval = SYSCFG->EXTICR[(linepos >> 2U) & 0x03UL];
      regval &= ~(SYSCFG_EXTICR1_EXTI0
                  << (SYSCFG_EXTICR1_EXTI1_Pos * (linepos & 0x03UL)));
      SYSCFG->EXTICR[(linepos >> 2U) & 0x03UL] = regval;
    }
  }
