...
#define BDMA_PERIPH_TO_MEMORY                                                  \
  (0x00000000U) /*!< Peripheral to memory direction */
#define BDMA_MEMORY_TO_PERIPH                                                  \
  ((uint32_t)BDMA_CCR_DIR) /*!< Memory to peripheral direction */
#define BDMA_MEMORY_TO_MEMORY                                                  \
  ((uint32_t)BDMA_CCR_MEM2MEM) /*!< Memory to memory direction     */
...
#define DMA_TO_BDMA_PERIPHERAL_INC(__DMA_PERIPHERAL_INC__)                     \
  ((__DMA_PERIPHERAL_INC__) >> 3U)
...
#define IS_DMA_UART_USART_REQUEST(__REQUEST__)                                 \
  ((((__REQUEST__) >= DMA_REQUEST_USART1_RX) &&                                \
    ((__REQUEST__) <= DMA_REQUEST_USART3_TX)) ||                               \
   (((__REQUEST__) >= DMA_REQUEST_UART4_RX) &&                                 \
    ((__REQUEST__) <= DMA_REQUEST_UART5_TX)) ||                                \
   (((__REQUEST__) >= DMA_REQUEST_USART6_RX) &&                                \
    ((__REQUEST__) <= DMA_REQUEST_USART6_TX)) ||                               \
   (((__REQUEST__) >= DMA_REQUEST_UART7_RX) &&                                 \
    ((__REQUEST__) <= DMA_REQUEST_UART8_TX)) ||                                \
   (((__REQUEST__) >= DMA_REQUEST_UART9_RX) &&                                 \
    ((__REQUEST__) <= DMA_REQUEST_USART10_TX)))
#else
#define IS_DMA_UART_USART_REQUEST(__REQUEST__)                                 \
  ((((__REQUEST__) >= DMA_REQUEST_USART1_RX) &&                                \
    ((__REQUEST__) <= DMA_REQUEST_USART3_TX)) ||                               \
...
HAL_StatusTypeDef HAL_DMA_Init(DMA_HandleTypeDef *hdma) {
  ...
  if (IS_DMA_STREAM_INSTANCE(hdma->Instance) != 0U) /* DMA1 or DMA2 instance */
  {
    ...
    /* Work around for Errata 2.22: UART/USART- DMA transfer lock: DMA stream
       could be lock when transferring data to/from USART/UART */
  ...
  } else if (IS_BDMA_CHANNEL_INSTANCE(hdma->Instance) !=
             0U) /* BDMA instance(s) */
  {
     ...
     */
    registerValue &= ((uint32_t) ~(
        BDMA_CCR_PL | BDMA_CCR_MSIZE | BDMA_CCR_PSIZE | BDMA_CCR_MINC |
        BDMA_CCR_PINC | BDMA_CCR_CIRC | BDMA_CCR_DIR | BDMA_CCR_MEM2MEM |
        BDMA_CCR_DBM | BDMA_CCR_CT));
    ...
    hdma->StreamIndex =
        (((uint32_t)((uint32_t *)hdma->Instance) - (uint32_t)BDMA_Channel0) /
  ...
  if (IS_DMA_DMAMUX_ALL_INSTANCE(hdma->Instance) !=
...
#if defined(BDMA1) /* No DMAMUX available for BDMA1 available on  STM32H7Ax/Bx \
                      devices only */
  if (IS_DMA_DMAMUX_ALL_INSTANCE(hdma->Instance) !=
      0U) /* No DMAMUX available for BDMA1 */
#endif    /* BDMA1 */
...
HAL_StatusTypeDef HAL_DMA_Start_IT(DMA_HandleTypeDef *hdma, uint32_t SrcAddress,
  ...
  if (HAL_DMA_STATE_READY == hdma->State) {
    ...
    if (IS_DMA_STREAM_INSTANCE(hdma->Instance) !=
...
void HAL_DMA_IRQHandler(DMA_HandleTypeDef *hdma) {
  ...
  if (IS_DMA_STREAM_INSTANCE(hdma->Instance) != 0U) /* DMA1 or DMA2 instance */
  {
    ...
    if (hdma->ErrorCode != HAL_DMA_ERROR_NONE) {
      if ((hdma->ErrorCode & HAL_DMA_ERROR_TE) != 0U) {
        ...
        } while ((((DMA_Stream_TypeDef *)hdma->Instance)->CR & DMA_SxCR_EN) !=
                 0U);
             ...
             0U) /* BDMA instance(s) */
  {
    ...
    if (((tmpisr_bdma & (BDMA_FLAG_HT0 << (hdma->StreamIndex & 0x1FU))) !=
         0U) &&
        ((ccr_reg & BDMA_CCR_HTIE) != 0U)) {
      /* Clear the half transfer complete flag */
      regs_bdma->IFCR = (BDMA_ISR_HTIF0 << (hdma->StreamIndex & 0x1FU));

      /* Disable the transfer complete interrupt if the DMA mode is Double
       * Buffering */
     ...
     ***********************************/
      ...
      if ((ccr_reg & BDMA_CCR_DBM) != 0U) {
        ...
        if ((ccr_reg & BDMA_CCR_CIRC) == 0U) {
          /* Disable the transfer complete and error interrupt, if the DMA mode
           * is not CIRCULAR */
...
static uint32_t DMA_CalcBaseAndBitshift(DMA_HandleTypeDef *hdma) {
  ...
  } else /* BDMA instance(s) */
  {
    ...
    hdma->StreamBaseAddress =
        ((uint32_t)((uint32_t *)hdma->Instance) & (uint32_t)(~0xFFU));
  }
...
static void DMA_CalcDMAMUXChannelBaseAndMask(DMA_HandleTypeDef *hdma) {
  ...
  if (IS_BDMA_CHANNEL_DMAMUX_INSTANCE(hdma->Instance) != 0U) {
    ...
    stream_number =
        (((uint32_t)((uint32_t *)hdma->Instance) & 0xFFU) - 8U) / 20U;
