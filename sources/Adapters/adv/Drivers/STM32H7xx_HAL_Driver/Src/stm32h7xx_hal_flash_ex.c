...
/**
 ...
 erase
      ...
      (#) FLASH Memory Lock and unlock per Bank: HAL_FLASHEx_Lock_Bank1(),
 HAL_FLASHEx_Unlock_Bank1(), HAL_FLASHEx_Lock_Bank2() and
 HAL_FLASHEx_Unlock_Bank2() functions
      ...
      (#) Error correction code error functions:
          (++) Use the HAL_FLASHEx_EnableEccCorrectionInterrupt() and
 HAL_FLASHEx_DisableEccCorrectionInterrupt() functions to enable and disable the
 FLASH ECC correction interruption.
          (++) Use the HAL_FLASHEx_EnableEccDetectionInterrupt() and
 HAL_FLASHEx_DisableEccDetectionInterrupt() functions to enable and disable the
 FLASH ECC Detection interruption.
          (++) Handle ECCD interrupt by calling
 HAL_FLASHEx_BusFault_IRQHandler()
          (++) Use HAL_FLASHEx_BusFault_IRQHandler() function called under
 BusFault_IRQHandler() interrupt subroutine to handle the ECCD interrupt.
          (++) Use HAL_FLASHEx_GetEccInfo() function to get the flash ECC fail
 information.
...
static void FLASH_OB_GetSecureArea(uint32_t *SecureAreaConfig,
                                   uint32_t *SecureAreaStartAddr,
                                   uint32_t *SecureAreaEndAddr, uint32_t Bank);
...
HAL_StatusTypeDef HAL_FLASHEx_Erase_IT(FLASH_EraseInitTypeDef *pEraseInit) {
  ...
  if (status != HAL_OK) {
    ...
    if ((pEraseInit->Banks & FLASH_BANK_1) == FLASH_BANK_1) {
      ...
      __HAL_FLASH_ENABLE_IT_BANK1(FLASH_IT_EOP_BANK1 | FLASH_IT_WRPERR_BANK1 |
...
#if defined(FLASH_CR_OPERRIE)
      __HAL_FLASH_ENABLE_IT_BANK2(FLASH_IT_EOP_BANK2 | FLASH_IT_WRPERR_BANK2 |
...
HAL_StatusTypeDef HAL_FLASHEx_OBProgram(FLASH_OBProgramInitTypeDef *pOBInit) {
...
#if defined(DUAL_BANK)
  else if (FLASH_WaitForLastOperation((uint32_t)FLASH_TIMEOUT_VALUE,
                                      FLASH_BANK_2) != HAL_OK) {
    status = HAL_ERROR;
  }
#endif /* DUAL_BANK */
  ...
  if (status == HAL_OK) {
    ...
    if ((pOBInit->OptionType & OPTIONBYTE_CM7_BOOTADD) ==
    ...
    if ((pOBInit->OptionType & OPTIONBYTE_CM4_BOOTADD) ==
        OPTIONBYTE_CM4_BOOTADD) {
      FLASH_OB_CM4BootAddConfig(pOBInit->CM4BootConfig, pOBInit->CM4BootAddr0,
                                pOBInit->CM4BootAddr1);
    }
#else  /* Single Core*/
    ...
    if ((pOBInit->OptionType & OPTIONBYTE_SECURE_AREA) ==
      ...
      FLASH_OB_SecureAreaConfig(pOBInit->SecureAreaConfig,
                                pOBInit->SecureAreaStartAddr,
                                pOBInit->SecureAreaEndAddr, pOBInit->Banks);
    }
...
void HAL_FLASHEx_OBGetConfig(FLASH_OBProgramInitTypeDef *pOBInit) {
  ...
  {
    pOBInit->OptionType |=
        (OPTIONBYTE_WRP | OPTIONBYTE_PCROP | OPTIONBYTE_SECURE_AREA);
...
HAL_StatusTypeDef HAL_FLASHEx_ComputeCRC(FLASH_CRCInitTypeDef *pCRCInit,
  ...
  if (status == HAL_OK) {
    if (pCRCInit->Bank == FLASH_BANK_1) {
      ...
      /* Clear CRC flags in Status Register: CRC end of calculation and CRC read
       * error */
      FLASH->CCR1 |= (FLASH_CCR_CLR_CRCEND | FLASH_CCR_CLR_CRCRDERR);

      /* Clear current CRC result, program burst size and define memory area on
       * which CRC has to be computed */
      FLASH->CRCCR1 |=
          FLASH_CRCCR_CLEAN_CRC | pCRCInit->BurstSize | pCRCInit->TypeCRC;
      ...
      /* Clear CRC flags */
      __HAL_FLASH_CLEAR_FLAG_BANK1(FLASH_FLAG_CRCEND_BANK1 |
      ...
      /* Clear CRC flags */
      __HAL_FLASH_CLEAR_FLAG_BANK2(FLASH_FLAG_CRCEND_BANK2 |
...
void HAL_FLASHEx_GetEccInfo(FLASH_EccInfoTypeDef *pData) {
  ...
  errorflag =
      FLASH->SR1 & (FLASH_FLAG_SNECCERR_BANK1 | FLASH_FLAG_DBECCERR_BANK1);
  if (errorflag != 0U) {
    pData->Area = FLASH_ECC_AREA_USER_BANK1;
    pData->Address = ((((FLASH->ECC_FA1 & FLASH_ECC_FA_FAIL_ECC_ADDR)) *
                       FLASH_NB_32BITWORD_IN_FLASHWORD * 4) +
                      FLASH_BANK1_BASE);
  }
#if defined(DUAL_BANK)
  ...
  if (errorflag != 0U) {
    ...
    pData->Address = ((((FLASH->ECC_FA2 & FLASH_ECC_FA_FAIL_ECC_ADDR)) *
                       FLASH_NB_32BITWORD_IN_FLASHWORD * 4) +
                      FLASH_BANK2_BASE);
  }
#endif /* DUAL_BANK */
...
void FLASH_Erase_Sector(uint32_t Sector, uint32_t Banks,
  ...
  if ((Banks & FLASH_BANK_1) == FLASH_BANK_1) {
    ...
    FLASH->CR1 |=
        (FLASH_CR_SER | (Sector << FLASH_CR_SNB_Pos) | FLASH_CR_START);
#endif /* FLASH_CR_PSIZE */
  }
 ...
 *          This parameter can be one of the following values:
 *            @arg WRPSector:  A combination of OB_WRP_SECTOR_0 to
 * OB_WRP_SECTOR_7 or OB_WRP_SECTOR_ALL
 *
 ...
 *          This parameter can be one of the following values:
 *            @arg WRPSector:  A combination of FLASH_OB_WRP_SECTOR_0 to
 * FLASH_OB_WRP_SECTOR_7 or FLASH_OB_WRP_SECTOR_ALL
 *
 * @param  Banks the specific bank to apply WRP sectors
 *          This parameter can be one of the following values:
 ...
 *         IWDG1_SW(Bit4), IWDG2_SW(Bit 5), nRST_STOP_D1(Bit 6),
 * nRST_STDY_D1(Bit 7), FZ_IWDG_STOP(Bit 17), FZ_IWDG_SDBY(Bit 18),
 * ST_RAM_SIZE(Bit[19:20]), SECURITY(Bit 21), BCM4(Bit 22), BCM7(Bit 23),
 * nRST_STOP_D2(Bit 24), nRST_STDY_D2(Bit 25), IO_HSLV (Bit 29) and
 * SWAP_BANK_OPT(Bit 31).
 *
 * @retval HAL status
 */
#else
 ...
 *         through the call of the HAL_FLASH_OB_Launch() function.
 *
 * @param  PCROPConfig specifies if the PCROP area for the given Bank shall be
 * erased or not when RDP level decreased from Level 1 to Level 0, or after a
 * bank erase with protection removal This parameter must be a value of @arg
 ...
 *            @arg FLASH_BANK_BOTH: PCROP on specified bank1 and bank2 area
 * (same config will be applied on both banks)
 *
 * @retval None
 */
static void FLASH_OB_PCROPConfig(uint32_t PCROPConfig, uint32_t PCROPStartAddr,
  ...
  if ((Banks & FLASH_BANK_2) == FLASH_BANK_2) {
    ...
    FLASH->PRAR_PRG2 = ((PCROPStartAddr - FLASH_BANK2_BASE) >> 8) |
                       (((PCROPEndAddr - FLASH_BANK2_BASE) >> 8)
                        << FLASH_PRAR_PROT_AREA_END_Pos) |
                       PCROPConfig;
  }
#endif /* DUAL_BANK */
...
/**
 * @brief  Get the Proprietary code readout protection configuration on a given
 * Bank
 *
 * @param  PCROPConfig indicates if the PCROP area for the given Bank shall be
 * erased or not when RDP level decreased from Level 1 to Level 0 or after a
 * bank erase with protection removal
 *
 * @param  PCROPStartAddr gives the start address of the Proprietary code
 * readout protection of the bank
 *
 * @param  PCROPEndAddr gives the end address of the Proprietary code readout
...
static void FLASH_OB_GetPCROP(uint32_t *PCROPConfig, uint32_t *PCROPStartAddr,
  ...
  (*PCROPEndAddr) =
      (regvalue & FLASH_PRAR_PROT_AREA_END) >> FLASH_PRAR_PROT_AREA_END_Pos;
...
/**
  ...
  *                     This parameter must be a value of @ref
  FLASHEx_OB_BOOT_OPTION (OB_BOOT_ADD0, OB_BOOT_ADD1 or OB_BOOT_ADD_BOTH)
  *
 ...
 *         when RDP level decreased from Level 1 to Level 0 or during a mass
 * erase.
 *
 ...
 *            @arg FLASH_BANK_BOTH: Secure area on specified bank1 and bank2
 * area (same config will be applied on both banks)
 * @retval None
 */
static void FLASH_OB_SecureAreaConfig(uint32_t SecureAreaConfig,
                                      uint32_t SecureAreaStartAddr,
  ...
  if ((Banks & FLASH_BANK_2) == FLASH_BANK_2) {
    ...
    FLASH->SCAR_PRG2 = ((SecureAreaStartAddr - FLASH_BANK2_BASE) >> 8) |
                       (((SecureAreaEndAddr - FLASH_BANK2_BASE) >> 8)
                        << FLASH_SCAR_SEC_AREA_END_Pos) |
                       (SecureAreaConfig & FLASH_SCAR_DMES);
  }
#endif /* DUAL_BANK */
...
static void FLASH_OB_GetSecureArea(uint32_t *SecureAreaConfig,
  ...
  (*SecureAreaStartAddr) =
      ((regvalue & FLASH_SCAR_SEC_AREA_START) << 8) + bankBase;
  (*SecureAreaEndAddr) =
      (regvalue & FLASH_SCAR_SEC_AREA_END) >> FLASH_SCAR_SEC_AREA_END_Pos;
  (*SecureAreaEndAddr) = ((*SecureAreaEndAddr) << 8) + bankBase;
}

/**
 * @brief  Add a CRC sector to the list of sectors on which the CRC will be
 * calculated
...
/**
 * @brief  Select CRC start and end memory addresses on which the CRC will be
 * calculated
 ...
 */
static uint32_t FLASH_OB_OTP_GetLock(void) { return (FLASH->OTPBL_CUR); }
#endif /* FLASH_OTPBL_LOCKBL */
