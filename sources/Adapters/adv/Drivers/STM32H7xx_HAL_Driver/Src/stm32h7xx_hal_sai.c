  ...
  ******************************************************************************
  ...
  [..]
    (@) PDM interface can be activated through HAL_SAI_Init function.
        Please note that PDM interface is only available for SAI1 or SAI4
  sub-block A. PDM microphone delays can be tuned with
  HAL_SAIEx_ConfigPdmMicDelay function.
...
HAL_StatusTypeDef HAL_SAI_Init(SAI_HandleTypeDef *hsai) {
  ...
  if (hsai->Init.PdmInit.Activation == ENABLE) {
    ...
    /* Check that SAI sub-block is SAI1 or SAI4 sub-block A, in master RX mode
     * with free protocol */
#if defined(SAI4)
...
#endif /* SAI2 */
#if defined(SAI3)
  else if ((hsai->Instance == SAI3_Block_A) ||
           (hsai->Instance == SAI3_Block_B)) {
    SaiBaseAddress = SAI3;
  }
#endif /* SAI3 */
#if defined(SAI4)
  else if ((hsai->Instance == SAI4_Block_A) ||
           (hsai->Instance == SAI4_Block_B)) {
    SaiBaseAddress = SAI4;
  }
#endif /* SAI4 */
  else {
...
#if defined(SAI_VER_V2_X) /* SAI Peripheral version depends on STM32H7 device  \
                             revision ID */
