/**
  ...
  ******************************************************************************
  ...
  ==============================================================================
    [..] The Delay block is used to generate an Output clock which is de-phased
  ...
  drivers that interfaces with the DELAY peripheral. The DelayBlock_Enable()
  function, enables the DelayBlock instance, configure the delay line length and
  configure the Output clock phase. The DelayBlock_Disable() function, disables
...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
/** @defgroup DelayBlock_LL_Private_Defines Delay Block Low Layer Private
 * Defines
 * @{
 */
...
/** @defgroup DelayBlock_LL_Exported_Functions Delay Block Low Layer Exported
 * Functions
 * @{
 */
...
HAL_StatusTypeDef DelayBlock_Enable(DLYB_TypeDef *DLYBx) {
  ...
  for (sel_current = 0U; sel_current < DLYB_MAX_SELECT; sel_current++) {
    /* lng_mask is the mask bit for the LNG field to check the output of the
     * UNITx*/
    ...
    for (unit_current = 0U; unit_current < DLYB_MAX_UNIT; unit_current++) {
      ...
      if (tuning == 0U) {
        ...
        /* 1/2 period LOW detected after the HIGH 1/2 period => FULL PERIOD
         * passed*/
        if ((DLYBx->CFGR & lng_mask) == 0U) {
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef DelayBlock_Configure(DLYB_TypeDef *DLYBx, uint32_t PhaseSel,
                                       uint32_t Units) {
