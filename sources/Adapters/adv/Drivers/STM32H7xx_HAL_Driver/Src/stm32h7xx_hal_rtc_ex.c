/**
  ...
  *** Outputs configuration ***
  ...
  [..]  The RTC has 2 different outputs:
    ...
    (+) RTC_CALIB: this output is 512Hz signal or 1Hz.
        To enable the RTC_CALIB, use the HAL_RTCEx_SetCalibrationOutPut()
  function.
  ...
  *** Tamper configuration ***
  ...
  avoid erase, enable the NoErase field on the RTC_TAMPCR register.
...
HAL_StatusTypeDef HAL_RTCEx_SetTimeStamp(RTC_HandleTypeDef *hrtc,
  ...
  tmpreg =
      (uint32_t)(hrtc->Instance->CR & (uint32_t) ~(RTC_CR_TSEDGE | RTC_CR_TSE));
...
HAL_StatusTypeDef HAL_RTCEx_GetTimeStamp(RTC_HandleTypeDef *hrtc,
  ...
  sTimeStampDate->Date =
      (uint8_t)((tmpdate & (RTC_TSDR_DT | RTC_TSDR_DU)) >> RTC_TSDR_DU_Pos);
...
HAL_StatusTypeDef HAL_RTCEx_SetTamper(RTC_HandleTypeDef *hrtc,
  ...
  if ((hrtc->Instance->CR & RTC_CR_TAMPTS) !=
      (sTamper->TimeStampOnTamperDetection)) {
    __HAL_RTC_WRITEPROTECTION_DISABLE(hrtc);
...
HAL_StatusTypeDef HAL_RTCEx_SetTamper_IT(RTC_HandleTypeDef *hrtc,
  ...
  /* Clear the bits that are going to be configured and leave the others
   * unchanged */
  tmpreg &= ~((sTamper->Tamper << TAMP_CR2_TAMP1TRG_Pos) |
              (sTamper->Tamper << TAMP_CR2_TAMP1MSK_Pos) |
              (sTamper->Tamper << TAMP_CR2_TAMP1NOERASE_Pos));
...
HAL_StatusTypeDef HAL_RTCEx_SetTamper_IT(RTC_HandleTypeDef *hrtc,
  ...
  /* Configure the tamper trigger bit (located just next to the tamper enable
   * bit) */
  if ((sTamper->Trigger == RTC_TAMPERTRIGGER_HIGHLEVEL) ||
      (sTamper->Trigger == RTC_TAMPERTRIGGER_FALLINGEDGE)) {
    /* Set the tamper trigger bit */
...
HAL_StatusTypeDef HAL_RTCEx_DeactivateTamper(RTC_HandleTypeDef *hrtc,
  ...
  /* Clear the selected tamper flags in SR register by setting corresponding
   * bits in SCR register */
  tamp->SCR = Tamper;
  ...
  for (i = 0; i < RTC_TAMP_NB; i++) {
    if (sAllTamper->TampInput[i].Enable != RTC_ATAMP_DISABLE) {
      ...
      if (sAllTamper->TampInput[i].Output != i) {
        ...
        ATCR1 |= sAllTamper->TampInput[i].Output
                 << ((2u * i) + TAMP_ATCR1_ATOSEL1_Pos);
      }
    }
  }
...
void HAL_RTCEx_TamperTimeStampIRQHandler(RTC_HandleTypeDef *hrtc) {
  ...
  if ((hrtc->Instance->MISR & RTC_MISR_TSMF) != 0u) {
    ...
    /* Not immediately clear flags because the content of RTC_TSTR and RTC_TSDR
...
void HAL_RTCEx_TamperTimeStampIRQHandler(RTC_HandleTypeDef *hrtc) {
  ...
  if (__HAL_RTC_TIMESTAMP_GET_IT_SOURCE(hrtc, RTC_IT_TS) != 0U) {
    ...
    if (__HAL_RTC_TIMESTAMP_GET_FLAG(hrtc, RTC_FLAG_TSF) != 0U) {
      ...
      /* Clear the TIMESTAMP interrupt pending bit (this will clear timestamp
...
HAL_StatusTypeDef HAL_RTCEx_SetWakeUpTimer(RTC_HandleTypeDef *hrtc,
  ...
  /* Poll WUTWF until it is set in RTC_ICSR / RTC_ISR to make sure the access to
  wakeup autoreload counter and to WUCKSEL[2:0] bits is allowed. This step must
  be skipped in calendar initialization mode. */
#if defined(TAMP)
  ...
  /* Clear the Wakeup Timer clock source bits and configure the clock source in
   * CR register */
...
#if defined(TAMP)
  tmp = (uint32_t) &
        (((TAMP_TypeDef *)((uint32_t)hrtc->Instance + TAMP_OFFSET))->BKP0R);
#else
...
HAL_StatusTypeDef HAL_RTCEx_SetSynchroShift(RTC_HandleTypeDef *hrtc,
  ...
  if ((hrtc->Instance->CR & RTC_CR_REFCKON) == 0U) {
    ...
    hrtc->Instance->SHIFTR =
        (uint32_t)(uint32_t)(ShiftSubFS) | (uint32_t)(ShiftAdd1S);
