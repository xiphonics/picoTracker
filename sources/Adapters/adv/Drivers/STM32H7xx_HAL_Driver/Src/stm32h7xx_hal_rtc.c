/**
 ...
 : examples AlarmAEventCallback(), WakeUpTimerEventCallback(). Exception done
 for MspInit and MspDeInit callbacks that are reset to the legacy weak function
  ...
  if (hrtc != NULL) {
    ...
    if (__HAL_RTC_IS_CALENDAR_INITIALIZED(hrtc) == 0U) {
      ...
      if (status == HAL_OK) {
        ...
        hrtc->Instance->CR |= (hrtc->Init.OutPutPullUp | hrtc->Init.OutPutType |
...
HAL_StatusTypeDef HAL_RTC_DeInit(RTC_HandleTypeDef *hrtc) {
  ...
  if (hrtc != NULL) {
    ...
    if (status == HAL_OK) {
      ...
      hrtc->Instance->DR =
          ((uint32_t)(RTC_DR_WDU_0 | RTC_DR_MU_0 | RTC_DR_DU_0));
    ...
    if (status == HAL_OK) {
      ...
      ((TAMP_TypeDef *)((uint32_t)hrtc->Instance + TAMP_OFFSET))->CR1 =
          0xFFFF0000U;
      ((TAMP_TypeDef *)((uint32_t)hrtc->Instance + TAMP_OFFSET))->CR2 =
          0x00000000U;
#else
...
HAL_StatusTypeDef HAL_RTC_SetTime(RTC_HandleTypeDef *hrtc,
  ...
  if (status == HAL_OK) {
    if (Format == RTC_FORMAT_BIN) {
      ...
      tmpreg =
          (uint32_t)(((uint32_t)RTC_ByteToBcd2(sTime->Hours) << RTC_TR_HU_Pos) |
 ...
 * @param  hrtc RTC handle
 * @param  sTime Pointer to Time structure with Hours, Minutes and Seconds
 * fields returned with input format (BIN or BCD), also SubSeconds field
 * returning the RTC_SSR register content and SecondFraction field the
 ...
 * @note   The HAL_RTC_SetTime() must be called before enabling the Alarm
 * feature.
 * @retval HAL status
 */
...
__weak void HAL_RTC_AlarmAEventCallback(RTC_HandleTypeDef *hrtc) {
  ...
  /* NOTE : This function should not be modified, when the callback is needed,
            the HAL_RTC_AlarmAEventCallback could be implemented in the user
     file
   */
}
...
HAL_StatusTypeDef HAL_RTC_WaitForSynchro(RTC_HandleTypeDef *hrtc) {
  ...
  /* Clear RSF flag, keep reserved bits at reset values (setting other flags has
   * no effect) */
#if defined(TAMP)
