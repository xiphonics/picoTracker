...
HAL_StatusTypeDef HAL_SDRAM_Read_DMA(SDRAM_HandleTypeDef *hsdram,
  ...
  if (state == HAL_SDRAM_STATE_BUSY) {
    ...
    status =
        HAL_MDMA_Start_IT(hsdram->hmdma, (uint32_t)pAddress,
                          (uint32_t)pDstBuffer, (uint32_t)(BufferSize * 4U), 1);
...
HAL_StatusTypeDef HAL_SDRAM_Write_DMA(SDRAM_HandleTypeDef *hsdram,
  ...
  if (hsdram->State == HAL_SDRAM_STATE_BUSY) {
    ...
    /* Enable the DMA Stream */
    status =
        HAL_MDMA_Start_IT(hsdram->hmdma, (uint32_t)pSrcBuffer,
                          (uint32_t)pAddress, (uint32_t)(BufferSize * 4U), 1);
