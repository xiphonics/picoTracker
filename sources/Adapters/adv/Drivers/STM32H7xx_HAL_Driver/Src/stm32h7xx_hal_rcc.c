...
/**
 ...
 not derived from the System clock through
...
internal/external oscillators (HSE, HSI, LSE,CSI, LSI,HSI48, PLL, CSS and MCO)
and the System buses clocks (SYSCLK, AHB3, AHB1 AHB2,AHB4,APB3, APB1L, APB1H,
APB2, and APB4).
...
source.
         ...
         (#) PLL , The RCC features three independent PLLs (clocked by HSI , HSE
or CSI), featuring three different output clocks and able  to work either in
integer or Fractional mode.
           (++) A main PLL, PLL1, which is generally used to provide clocks to
the CPU and to some peripherals.
           (++) Two dedicated PLLs, PLL2 and PLL3, which are used to generate
the kernel clock for peripherals.


         (#) CSS (Clock security system), once enabled and if a HSE clock
...
enabled. The interrupt is linked to the Cortex-M NMI (Non-Mask-able Interrupt)
             exception vector.

         (#) MCO1 (micro controller clock output), used to output HSI, LSE, HSE,
PLL1(PLL1_Q) or HSI48 clock (through a configurable pre-scaler) on PA8 pin.

         (#) MCO2 (micro controller clock output), used to output HSE,
...
(SYSCLK): CSI,HSI, HSE and PLL. The AHB clock (HCLK) is derived from System core
clock through configurable pre-scaler and used to clock the CPU, memory and
peripherals mapped on AHB and APB bus of the 3 Domains (D1, D2, D3)* through
configurable pre-scalers and used to clock the peripherals mapped on these
buses. You can use "HAL_RCC_GetSysClockFreq()" function to retrieve system clock
frequency.

         -@- All the peripheral clocks are derived from the System clock
(SYSCLK) except those with dual clock domain where kernel source clock could be
selected through RCC_D1CCIPR,RCC_D2CCIP1R,RCC_D2CCIP2R and RCC_D3CCIPR
registers.
...
HAL_StatusTypeDef HAL_RCC_DeInit(void) {
  ...
  CLEAR_BIT(RCC->CR, RCC_CR_HSEON | RCC_CR_HSIKERON | RCC_CR_HSIDIV |
                         RCC_CR_HSIDIVF | RCC_CR_CSION | RCC_CR_CSIKERON |
  ...
  RCC->PLLCKSELR =
      RCC_PLLCKSELR_DIVM1_5 | RCC_PLLCKSELR_DIVM2_5 | RCC_PLLCKSELR_DIVM3_5;
 ...
 * Off first and then LSE On or LSE Bypass.
  ...
  if (((RCC_OscInitStruct->OscillatorType) & RCC_OSCILLATORTYPE_HSI) ==
    ...
    if ((temp_sysclksrc == RCC_CFGR_SWS_HSI) ||
      ...
      /* Otherwise, only HSI division and calibration are allowed */
        ...
        /* Enable the Internal High Speed oscillator (HSI, HSIDIV2, HSIDIV4, or
         * HSIDIV8) */
    ...
    if (__HAL_RCC_GET_SYSCLK_SOURCE() != RCC_CFGR_SWS_PLL1) {
      ...
      if (((RCC_OscInitStruct->PLL.PLLState) == RCC_PLL_OFF) ||
          ...
          ((READ_BIT(temp1_pllckcfg, RCC_PLLCKSELR_DIVM1) >>
            RCC_PLLCKSELR_DIVM1_Pos) != RCC_OscInitStruct->PLL.PLLM) ||
          (READ_BIT(temp2_pllckcfg, RCC_PLL1DIVR_N1) !=
           (RCC_OscInitStruct->PLL.PLLN - 1U)) ||
          ((READ_BIT(temp2_pllckcfg, RCC_PLL1DIVR_P1) >> RCC_PLL1DIVR_P1_Pos) !=
           (RCC_OscInitStruct->PLL.PLLP - 1U)) ||
          ((READ_BIT(temp2_pllckcfg, RCC_PLL1DIVR_Q1) >> RCC_PLL1DIVR_Q1_Pos) !=
           (RCC_OscInitStruct->PLL.PLLQ - 1U)) ||
          ((READ_BIT(temp2_pllckcfg, RCC_PLL1DIVR_R1) >> RCC_PLL1DIVR_R1_Pos) !=
 ...
 *
 * @note   The SystemCoreClock CMSIS variable is used to store System Core Clock
 * Frequency and updated by HAL_InitTick() function called within this function
 *
 ...
 * will occur when the clock source will be ready. You can use
 ...
 * correctly D1CPRE[3:0] bits to ensure that  Domain1 core clock not exceed the
...
HAL_RCC_ClockConfig(const RCC_ClkInitTypeDef *RCC_ClkInitStruct,
  ...
  if (((RCC_ClkInitStruct->ClockType) & RCC_CLOCKTYPE_D1PCLK1) ==
    ...
    if ((RCC_ClkInitStruct->APB3CLKDivider) >
      ...
      MODIFY_REG(RCC->CDCFGR1, RCC_CDCFGR1_CDPPRE,
                 RCC_ClkInitStruct->APB3CLKDivider);
    }
#endif
  }
  ...
  if (((RCC_ClkInitStruct->ClockType) & RCC_CLOCKTYPE_PCLK1) ==
    ...
    if ((RCC_ClkInitStruct->APB1CLKDivider) >
      ...
      assert_param(IS_RCC_PCLK1(RCC_ClkInitStruct->APB1CLKDivider));
      MODIFY_REG(RCC->CDCFGR2, RCC_CDCFGR2_CDPPRE1,
                 (RCC_ClkInitStruct->APB1CLKDivider));
    }
#endif
  }
  ...
  if (((RCC_ClkInitStruct->ClockType) & RCC_CLOCKTYPE_PCLK2) ==
    ...
    if ((RCC_ClkInitStruct->APB2CLKDivider) >
      ...
      assert_param(IS_RCC_PCLK2(RCC_ClkInitStruct->APB2CLKDivider));
      MODIFY_REG(RCC->CDCFGR2, RCC_CDCFGR2_CDPPRE2,
                 (RCC_ClkInitStruct->APB2CLKDivider));
    }
#endif
  }
  ...
  if (((RCC_ClkInitStruct->ClockType) & RCC_CLOCKTYPE_D3PCLK1) ==
    ...
    if ((RCC_ClkInitStruct->APB4CLKDivider) >
      ...
      assert_param(IS_RCC_D3PCLK1(RCC_ClkInitStruct->APB4CLKDivider));
      MODIFY_REG(RCC->SRDCFGR, RCC_SRDCFGR_SRDPPRE,
                 (RCC_ClkInitStruct->APB4CLKDivider));
    }
#endif
  }
  ...
  if (((RCC_ClkInitStruct->ClockType) & RCC_CLOCKTYPE_HCLK) ==
    ...
    if ((RCC_ClkInitStruct->AHBCLKDivider) >
      ...
      /* Set the new HCLK clock divider */
      assert_param(IS_RCC_HCLK(RCC_ClkInitStruct->AHBCLKDivider));
      MODIFY_REG(RCC->CDCFGR1, RCC_CDCFGR1_HPRE,
                 RCC_ClkInitStruct->AHBCLKDivider);
    }
#endif
  }
...
#else
    MODIFY_REG(RCC->CDCFGR1, RCC_CDCFGR1_CDCPRE,
               RCC_ClkInitStruct->SYSCLKDivider);
#endif
 ...
 * @retval None
 */
void HAL_RCC_EnableCSS(void) { SET_BIT(RCC->CR, RCC_CR_CSSHSEON); }

/**
 * @brief  Disables the Clock Security System.
 * @retval None
 */
void HAL_RCC_DisableCSS(void) { CLEAR_BIT(RCC->CR, RCC_CR_CSSHSEON); }
 ...
 * factors.
 * @note     (*) CSI_VALUE is a constant defined in stm32h7xx_hal_conf.h file
 * (default value 4 MHz) but the real value may vary depending on the variations
 *               in voltage and temperature.
 * @note     (**) HSI_VALUE is a constant defined in stm32h7xx_hal_conf.h file
 * (default value 64 MHz) but the real value may vary depending on the
 * variations in voltage and temperature.
 * @note     (***) HSE_VALUE is a constant defined in stm32h7xx_hal_conf.h file
...
uint32_t HAL_RCC_GetSysClockFreq(void) {
  ...
  case RCC_CFGR_SWS_HSI: /* HSI used as system clock source */
    ...
    if (__HAL_RCC_GET_FLAG(RCC_FLAG_HSIDIV) != 0U) {
      sysclockfreq =
          (uint32_t)(HSI_VALUE >> (__HAL_RCC_GET_HSI_DIVIDER() >> 3));
    } else {
 ...
 * will be incorrect.
 *
 * @note   The SystemD2Clock CMSIS variable is used to store System domain2
 * Clock Frequency and updated within this function
...
uint32_t HAL_RCC_GetPCLK1Freq(void) {
  ...
  /* Get HCLK source and Compute PCLK1 frequency ---------------------------*/
  return (HAL_RCC_GetHCLKFreq() >>
          ((D1CorePrescTable[(RCC->CDCFGR2 & RCC_CDCFGR2_CDPPRE1) >>
...
#else
  return (HAL_RCC_GetHCLKFreq() >>
          ((D1CorePrescTable[(RCC->CDCFGR2 & RCC_CDCFGR2_CDPPRE2) >>
...
void HAL_RCC_GetOscConfig(RCC_OscInitTypeDef *RCC_OscInitStruct) {
  ...
  } else if ((RCC->CR & (RCC_CR_HSEBYP | RCC_CR_HSEEXT)) ==
             (RCC_CR_HSEBYP | RCC_CR_HSEEXT)) {
  ...
  if (HAL_GetREVID() <= REV_ID_Y) {
    RCC_OscInitStruct->CSICalibrationValue =
        (uint32_t)(READ_BIT(RCC->HSICFGR, HAL_RCC_REV_Y_CSITRIM_Msk) >>
  ...
  if (HAL_GetREVID() <= REV_ID_Y) {
    RCC_OscInitStruct->HSICalibrationValue =
        (uint32_t)(READ_BIT(RCC->HSICFGR, HAL_RCC_REV_Y_HSITRIM_Msk) >>
  ...
  } else if ((RCC->BDCR & (RCC_BDCR_LSEBYP | RCC_BDCR_LSEEXT)) ==
             (RCC_BDCR_LSEBYP | RCC_BDCR_LSEEXT)) {
  ...
  RCC_OscInitStruct->PLL.PLLM =
      (uint32_t)((RCC->PLLCKSELR & RCC_PLLCKSELR_DIVM1) >>
                 RCC_PLLCKSELR_DIVM1_Pos);
  RCC_OscInitStruct->PLL.PLLN =
      (uint32_t)((RCC->PLL1DIVR & RCC_PLL1DIVR_N1) >> RCC_PLL1DIVR_N1_Pos) + 1U;
  RCC_OscInitStruct->PLL.PLLR =
      (uint32_t)((RCC->PLL1DIVR & RCC_PLL1DIVR_R1) >> RCC_PLL1DIVR_R1_Pos) + 1U;
  RCC_OscInitStruct->PLL.PLLP =
      (uint32_t)((RCC->PLL1DIVR & RCC_PLL1DIVR_P1) >> RCC_PLL1DIVR_P1_Pos) + 1U;
  RCC_OscInitStruct->PLL.PLLQ =
      (uint32_t)((RCC->PLL1DIVR & RCC_PLL1DIVR_Q1) >> RCC_PLL1DIVR_Q1_Pos) + 1U;
  RCC_OscInitStruct->PLL.PLLRGE =
      (uint32_t)((RCC->PLLCFGR & RCC_PLLCFGR_PLL1RGE));
  RCC_OscInitStruct->PLL.PLLVCOSEL =
      (uint32_t)((RCC->PLLCFGR & RCC_PLLCFGR_PLL1VCOSEL) >>
                 RCC_PLLCFGR_PLL1VCOSEL_Pos);
  RCC_OscInitStruct->PLL.PLLFRACN = (uint32_t)((
      (RCC->PLL1FRACR & RCC_PLL1FRACR_FRACN1) >> RCC_PLL1FRACR_FRACN1_Pos));
}
...
void HAL_RCC_GetClockConfig(RCC_ClkInitTypeDef *RCC_ClkInitStruct,
   ...
   * ----------------------------------------------*/
  RCC_ClkInitStruct->SYSCLKDivider =
      (uint32_t)(RCC->D1CFGR & RCC_D1CFGR_D1CPRE);
  ...
  RCC_ClkInitStruct->APB3CLKDivider =
      (uint32_t)(RCC->D1CFGR & RCC_D1CFGR_D1PPRE);

  /* Get the APB1 configuration ----------------------------------------------*/
  RCC_ClkInitStruct->APB1CLKDivider =
      (uint32_t)(RCC->D2CFGR & RCC_D2CFGR_D2PPRE1);

  /* Get the APB2 configuration ----------------------------------------------*/
  RCC_ClkInitStruct->APB2CLKDivider =
      (uint32_t)(RCC->D2CFGR & RCC_D2CFGR_D2PPRE2);
   ...
   * ----------------------------------------------*/
  RCC_ClkInitStruct->SYSCLKDivider =
      (uint32_t)(RCC->CDCFGR1 & RCC_CDCFGR1_CDCPRE);

  /* Get the D1HCLK configuration
   * ----------------------------------------------*/
  RCC_ClkInitStruct->AHBCLKDivider =
      (uint32_t)(RCC->CDCFGR1 & RCC_CDCFGR1_HPRE);

  /* Get the APB3 configuration ----------------------------------------------*/
  RCC_ClkInitStruct->APB3CLKDivider =
      (uint32_t)(RCC->CDCFGR1 & RCC_CDCFGR1_CDPPRE);

  /* Get the APB1 configuration ----------------------------------------------*/
  RCC_ClkInitStruct->APB1CLKDivider =
      (uint32_t)(RCC->CDCFGR2 & RCC_CDCFGR2_CDPPRE1);

  /* Get the APB2 configuration ----------------------------------------------*/
  RCC_ClkInitStruct->APB2CLKDivider =
      (uint32_t)(RCC->CDCFGR2 & RCC_CDCFGR2_CDPPRE2);

  /* Get the APB4 configuration ----------------------------------------------*/
  RCC_ClkInitStruct->APB4CLKDivider =
      (uint32_t)(RCC->SRDCFGR & RCC_SRDCFGR_SRDPPRE);
#endif
