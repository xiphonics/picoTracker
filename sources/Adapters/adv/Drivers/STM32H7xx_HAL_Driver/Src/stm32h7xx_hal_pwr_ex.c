...
/**
  ...
  ******************************************************************************
  ...
  [..]
   ...
   (#) Call HAL_PWREx_GetSupplyConfig() function to get the current supply
  setup.
...
#define PWR_EXTI_WAKEUP_PINS_MASK                                              \
  (EXTI_IMR2_IM55 | EXTI_IMR2_IM56 | EXTI_IMR2_IM57 | EXTI_IMR2_IM58 |         \
 ...
 *         regulator.
 * @note   This API is deprecated and is kept only for backward compatibility's
 * sake. The power supply configuration is handled as part of the system
 * initialization process during startup. For more details, please refer to the
 * power control chapter in the reference manual
 * @retval HAL status.
 */
   ...
   *** Keep D3/SRD in RUN mode ***
    ...
    *** Wakeup Pins configuration ****
    ...
    [..]
      ...
      his own code by customization of function pointer
HAL_PWREx_WKUPx_Callback.
...
void HAL_PWREx_EnableWakeUpPin(const PWREx_WakeupPinTypeDef *sPinParams) {
  ...
  pinConfig =
      ...
      (sPinParams->PinPolarity
       << ((POSITION_VAL(sPinParams->WakeUpPin) + PWR_WKUPEPR_WKUPP1_Pos) &
           0x1FU)) |
  ...
  MODIFY_REG(EXTI->IMR2, PWR_EXTI_WAKEUP_PINS_MASK,
             (sPinParams->WakeUpPin << EXTI_IMR2_IM55_Pos));
#endif /* !DUAL_CORE */
}
