/**
  ...
  *         functionalities of the Master Direct Memory Access (MDMA)
  peripheral:
  ...
  ******************************************************************************
  ...
  peripherals and MDMA requests.

   (#)
       For a given Channel use HAL_MDMA_Init function to program the required
  configuration through the following parameters: transfer request , channel
  priority, data endianness, Source increment, destination increment , source
  data size, destination data size, data alignment, source Burst, destination
  Burst , buffer Transfer Length, Transfer Trigger Mode (buffer transfer, block
  transfer, repeated block transfer or full transfer) source and destination
  block address offset, mask address and data.

       If using the MDMA in linked list mode then use function
  HAL_MDMA_LinkedList_CreateNode to fill a transfer node. Note that parameters
  given to the function HAL_MDMA_Init corresponds always to the node zero. Use
  function HAL_MDMA_LinkedList_AddNode to connect the created node to the linked
  list at a given position. User can make a linked list circular using function
  HAL_MDMA_LinkedList_EnableCircularMode , this function will automatically
  connect the last node of the list to the first one in order to make the list
  circular. In this case the linked list will loop on node 1 : first node
  connected after the initial transfer defined by the HAL_MDMA_Init

      -@-   The initial transfer itself (node 0 corresponding to the Init).
            User can disable the circular mode using function
  HAL_MDMA_LinkedList_DisableCircularMode, this function will then remove the
  connection between last node and first one.

       Function HAL_MDMA_LinkedList_RemoveNode can be used to remove
  (disconnect) a node from the transfer linked list. When a linked list is
  circular (last node connected to first one), if removing node1  (node where
  the linked list loops), the linked list remains circular and node 2 becomes
  the first one. Note that if the linked list is made circular the transfer will
  loop infinitely (or until aborted by the user).

    [..]
       (+) User can select the transfer trigger mode (parameter
  TransferTriggerMode) to define the amount of data to be transfer upon a
  request :
             (++) MDMA_BUFFER_TRANSFER : each request triggers a transfer of
  BufferTransferLength data with BufferTransferLength defined within the
  HAL_MDMA_Init.
             (++) MDMA_BLOCK_TRANSFER : each request triggers a transfer of a
  block with block size defined within the function
  HAL_MDMA_Start/HAL_MDMA_Start_IT or within the current linked list node
  parameters.
             (++) MDMA_REPEAT_BLOCK_TRANSFER : each request triggers a transfer
  of a number of blocks with block size and number of blocks defined within the
  function HAL_MDMA_Start/HAL_MDMA_Start_IT or within the current linked list
  node parameters.
  ...
  HAL_MDMA_LinkedList_CreateNode \ HAL_MDMA_LinkedList_AddNode).
     ...
     *** Polling mode IO operation ***
     =================================
    [..]
          (+) Use HAL_MDMA_Start() to start MDMA transfer after the
  configuration of Source address and destination address and the Length of data
  to be transferred.
          (+) Use HAL_MDMA_PollForTransfer() to poll for the end of current
  transfer or a transfer level In this case a fixed Timeout can be configured by
  User depending from his application.
          (+) Use HAL_MDMA_Abort() function to abort the current transfer :
  blocking method this API returns when the abort ends or timeout (should not be
  called from an interrupt service routine).
    ...
    [..]
          ...
          (+) Use HAL_MDMA_Start_IT() to start MDMA transfer after the
  configuration of Source address and destination address and the Length of data
  to be transferred. In this case the MDMA interrupt is configured.
          (+) Use HAL_MDMA_IRQHandler() called under MDMA_IRQHandler() Interrupt
  subroutine
          (+) At the end of data transfer HAL_MDMA_IRQHandler() function is
  executed and user can add his own function by customization of function
  pointer XferCpltCallback and XferErrorCallback (i.e a member of MDMA handle
  structure).

          (+) Use HAL_MDMA_Abort_IT() function to abort the current transfer :
  non-blocking method. This API will finish the execution immediately then the
  callback XferAbortCallback (if specified  by the user) is asserted once the
  MDMA channel has effectively aborted. (could be called from an interrupt
  service routine).

          (+) Use functions HAL_MDMA_RegisterCallback and
  HAL_MDMA_UnRegisterCallback respectevely to register unregister user callbacks
              from the following list :
              (++) XferCpltCallback            : transfer complete callback.
              (++) XferBufferCpltCallback      : buffer transfer complete
  callback.
              (++) XferBlockCpltCallback       : block transfer complete
  callback.
              (++) XferRepeatBlockCpltCallback : repeated block transfer
  complete callback.
              (++) XferErrorCallback           : transfer error callback.
              (++) XferAbortCallback           : transfer abort complete
  callback.

    [..]
         (+)  If the transfer Request corresponds to SW request
  (MDMA_REQUEST_SW) User can use function HAL_MDMA_GenerateSWRequest to trigger
  requests manually. Function HAL_MDMA_GenerateSWRequest must be used with the
  following precautions:
              (++) This function returns an error if used while the Transfer has
  ended or not started.
              (++) If used while the current request has not been served yet
  (current request transfer on going) this function returns an error and the new
  request is ignored.

              Generally this function should be used in conjunctions with the
  MDMA callbacks:
              (++) example 1:
                 (+++) Configure a transfer with request set to MDMA_REQUEST_SW
  and trigger mode set to MDMA_BUFFER_TRANSFER
                 (+++) Register a callback for buffer transfer complete (using
  callback ID set to HAL_MDMA_XFER_BUFFERCPLT_CB_ID)
                 (+++) After calling HAL_MDMA_Start_IT the MDMA will issue the
  transfer of a first BufferTransferLength data.
                 (+++) When the buffer transfer complete callback is asserted
  first buffer has been transferred and user can ask for a new buffer transfer
                   request using HAL_MDMA_GenerateSWRequest.

              (++) example 2:
                 (+++) Configure a transfer with request set to MDMA_REQUEST_SW
  and trigger mode set to MDMA_BLOCK_TRANSFER
                 (+++) Register a callback for block transfer complete (using
  callback ID HAL_MDMA_XFER_BLOCKCPLT_CB_ID)
                 (+++) After calling HAL_MDMA_Start_IT the MDMA will issue the
  transfer of a first block of data.
                 (+++) When the block transfer complete callback is asserted the
  first block has been transferred and user can ask for a new block transfer
  request using HAL_MDMA_GenerateSWRequest.

    [..]  Use HAL_MDMA_GetState() function to return the MDMA state and
  HAL_MDMA_GetError() in case of error detection.
     ...
     *** MDMA HAL driver macros list ***
     ...
     [..]
      ...
      (+) __HAL_MDMA_GET_IT_SOURCE: Check whether the specified MDMA Channel
  interrupt has occurred or not.

     [..]
      (@) You can refer to the header file of the MDMA HAL driver for more
  useful macros.
...
#define HAL_MDMA_CHANNEL_SIZE                                                  \
  0x40U /* an MDMA instance channel size is 64 byte  */
/**
 * @}
 */
/* Private macro -------------------------------------------------------------*/
...
@verbatim
 ...
 ===============================================================================
    ...
    [..]
    The HAL_MDMA_Init() function follows the MDMA channel configuration
procedures as described in reference manual. The HAL_MDMA_DeInit function allows
to deinitialize the MDMA channel. HAL_MDMA_RegisterCallback and
HAL_MDMA_UnRegisterCallback functions allows respectevely to register/unregister
an MDMA callback function.
...
HAL_StatusTypeDef HAL_MDMA_DeInit(MDMA_HandleTypeDef *hmdma) {
  ...
  /* Clear all flags */
  __HAL_MDMA_CLEAR_FLAG(hmdma, (MDMA_FLAG_TE | MDMA_FLAG_CTC | MDMA_FLAG_BRT |
 ...
 * MDMA Channel.
 * @param  MaskAddress: specifies the address to be updated (written) with
 * MaskData after a request is served.
 * @param  MaskData:    specifies the value to be written to MaskAddress after a
 * request is served. MaskAddress and MaskData could be used to automatically
 ...
 * @param  pNode: Pointer to a MDMA_LinkNodeTypeDef structure that contains
 * Linked list node registers configurations.
 * @param  pNodeConfig: Pointer to a MDMA_LinkNodeConfTypeDef structure that
 * contains the configuration information for the specified MDMA Linked List
 * Node.
 * @retval HAL status
 */
HAL_StatusTypeDef
HAL_MDMA_LinkedList_CreateNode(MDMA_LinkNodeTypeDef *pNode,
  ...
  pNode->CTCR =
      pNodeConfig->Init.SourceInc | pNodeConfig->Init.DestinationInc |
  ...
  */
  ...
  pNode->CBNDTR =
      ((pNodeConfig->BlockCount - 1U) << MDMA_CBNDTR_BRC_Pos) & MDMA_CBNDTR_BRC;

  /* if block source address offset is negative set the Block Repeat Source
   * address Update Mode to decrement */
  ...
  /* if block destination address offset is negative set the Block Repeat
   * destination address Update Mode to decrement */
  ...
  /* if HW request set the HW request and the requet CleraMask and ClearData
   * MaskData,  */
  if (pNodeConfig->Init.Request != MDMA_REQUEST_SW) {
 ...
 * Channel.
 * @param  pNewNode : Pointer to a MDMA_LinkNodeTypeDef structure that contains
 * Linked list node to be add to the list.
 * @param pPrevNode : Pointer to the new node position in the linked list or
 * zero to insert the new node at the end of the list
 *
 * @retval HAL status
 */
HAL_StatusTypeDef
    ...
    if ((uint32_t)hmdma->FirstLinkedListNodeAddress == 0U) {
      ...
      while ((counter < hmdma->LinkedListNodeCounter) &&
        ...
        if (pNode->CLAR == (uint32_t)pNewNode) {
          hal_status = HAL_ERROR; /* error this node already exist in the linked
                                     list and it is not first node */
        }
...
HAL_StatusTypeDef HAL_MDMA_LinkedList_RemoveNode(MDMA_HandleTypeDef *hmdma,
  ...
  if (HAL_MDMA_STATE_READY == hmdma->State) {
    ...
    if (((uint32_t)hmdma->FirstLinkedListNodeAddress == 0U) ||
      ...
      hal_status = HAL_ERROR;
    } else if (hmdma->FirstLinkedListNodeAddress ==
               pNode) /* Deleting first node */
    {
      /* Delete 1st node */
      if (hmdma->LastLinkedListNodeAddress == pNode) {
        ...
        if ((uint32_t)hmdma->FirstLinkedListNodeAddress ==
          ...
          /* if last node is looping to first (circular list) one update the
...
/**
 * @brief  Make the linked list circular by connecting the last node to the
 * first.
 * @param  hmdma : Pointer to a MDMA_HandleTypeDef structure that contains
 *                 the configuration information for the specified MDMA Channel.
 * @retval HAL status
...
/**
 * @brief  Disable the linked list circular mode by setting the last node
 * connection to null
 * @param  hmdma : Pointer to a MDMA_HandleTypeDef structure that contains
 *                 the configuration information for the specified MDMA Channel.
 * @retval HAL status
...
HAL_StatusTypeDef HAL_MDMA_Start_IT(MDMA_HandleTypeDef *hmdma,
  ...
  if (HAL_MDMA_STATE_READY == hmdma->State) {
    ...
    /* Enable Common interrupts i.e Transfer Error IT and Channel Transfer
     * Complete IT*/
    __HAL_MDMA_ENABLE_IT(hmdma, (MDMA_IT_TE | MDMA_IT_CTC));

    if (hmdma->XferBlockCpltCallback != NULL) {
      /* if Block transfer complete Callback is set enable the corresponding
       * IT*/
    ...
    if (hmdma->XferBufferCpltCallback != NULL) {
      /* if buffer transfer complete Callback is set enable the corresponding
       * IT*/
 ...
 *                 the configuration information for the specified MDMA Channel.
 *
 * @note  After disabling a MDMA Channel, a check for wait until the MDMA
 * Channel is effectively disabled is added. If a Channel is disabled while a
 * data transfer is ongoing, the current data will be transferred and the
 * Channel will be effectively disabled only after the transfer of this single
 * data is finished.
 * @retval HAL status
 */
...
HAL_MDMA_PollForTransfer(MDMA_HandleTypeDef *hmdma,
  ...
  while (__HAL_MDMA_GET_FLAG(hmdma, levelFlag) == 0U) {
    if ((__HAL_MDMA_GET_FLAG(hmdma, MDMA_FLAG_TE) != 0U)) {
      ...
      (void)HAL_MDMA_Abort(
          hmdma); /* if error then abort the current transfer */
...
void HAL_MDMA_IRQHandler(MDMA_HandleTypeDef *hmdma) {
  ...
  generalIntFlag =
      1UL << ((((uint32_t)hmdma->Instance - (uint32_t)(MDMA_Channel0)) /
...
static void MDMA_Init(MDMA_HandleTypeDef *hmdma) {
  ...
  /* Write new CTCR Register value */
  hmdma->Instance->CTCR =
      hmdma->Init.SourceInc | hmdma->Init.DestinationInc |
  ...
  if (hmdma->Init.DestBlockAddressOffset < 0) {
    ...
    hmdma->Instance->CBRUR |=
        ((blockoffset & 0x0000FFFFU) << MDMA_CBRUR_DUV_Pos);
  } else {
    /*write new CBRUR Register value : destination repeat block offset */
    hmdma->Instance->CBRUR |=
