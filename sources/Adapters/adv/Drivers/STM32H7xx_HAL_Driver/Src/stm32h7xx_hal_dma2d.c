/**
  ...
  ******************************************************************************
  ...
  to the default weak (overridden) function.
  ...
  (overridden) functions: examples @ref HAL_DMA2D_LineEventCallback(), @ref
  HAL_DMA2D_CLUTLoadingCpltCallback() Exception done for MspInit and MspDeInit
  callbacks that are respectively reset to the legacy weak (overridden)
  ...
  that are not defined as weak (overridden) functions. They must be defined by
...
HAL_StatusTypeDef HAL_DMA2D_Init(DMA2D_HandleTypeDef *hdma2d) {
   ...
   * -------------------------------------------*/
  MODIFY_REG(hdma2d->Instance->CR, DMA2D_CR_MODE | DMA2D_CR_LOM,
...
HAL_StatusTypeDef HAL_DMA2D_DeInit(DMA2D_HandleTypeDef *hdma2d) {
  ...
  if (__HAL_RCC_DMA2D_IS_CLK_ENABLED() == 1U) {
    ...
    if ((hdma2d->Instance->CR & DMA2D_CR_START) == DMA2D_CR_START) {
      ...
      if ((hdma2d->Instance->BGPFCCR & DMA2D_BGPFCCR_START) ==
          DMA2D_BGPFCCR_START) {
 ...
 *         DMA2D Callback is redirected to the weak (overridden) predefined
 * callback
 * @param hdma2d DMA2D handle
  ...
  if (HAL_DMA2D_STATE_READY == hdma2d->State) {
    ...
    case HAL_DMA2D_MSPINIT_CB_ID:
      hdma2d->MspInitCallback =
          HAL_DMA2D_MspInit; /* Legacy weak (overridden) Msp Init */
      break;
...
HAL_StatusTypeDef HAL_DMA2D_Abort(DMA2D_HandleTypeDef *hdma2d) {
  ...
  /* START bit is reset to make sure not to set it again, in the event the HW
     clears it between the register read and the register write by the CPU
     (writing 0 has no effect on START bitvalue) */
  MODIFY_REG(hdma2d->Instance->CR, DMA2D_CR_ABORT | DMA2D_CR_START,
             DMA2D_CR_ABORT);
...
HAL_StatusTypeDef HAL_DMA2D_Suspend(DMA2D_HandleTypeDef *hdma2d) {
  ...
  /* START bit is reset to make sure not to set it again, in the event the HW
     clears it between the register read and the register write by the CPU
     (writing 0 has no effect on START bitvalue). */
  MODIFY_REG(hdma2d->Instance->CR, DMA2D_CR_SUSP | DMA2D_CR_START,
             DMA2D_CR_SUSP);
  ...
  while ((hdma2d->Instance->CR & (DMA2D_CR_SUSP | DMA2D_CR_START)) ==
         DMA2D_CR_START) {
  ...
  /* Check whether or not a transfer is actually suspended and change the DMA2D
   * state accordingly */
...
HAL_DMA2D_CLUTStartLoad_IT(DMA2D_HandleTypeDef *hdma2d,
  ...
  if (LayerIdx == DMA2D_BACKGROUND_LAYER) {
    ...
    /* Enable the CLUT Transfer Complete, transfer Error, configuration Error
     * and CLUT Access Error interrupts */
    __HAL_DMA2D_ENABLE_IT(hdma2d, DMA2D_IT_CTC | DMA2D_IT_TE | DMA2D_IT_CE |
                                      DMA2D_IT_CAE);
...
HAL_StatusTypeDef HAL_DMA2D_CLUTLoading_Abort(DMA2D_HandleTypeDef *hdma2d,
  ...
  /* Disable the CLUT Transfer Complete, Transfer Error, Configuration Error and
   * CLUT Access Error interrupts */
  __HAL_DMA2D_DISABLE_IT(hdma2d, DMA2D_IT_CTC | DMA2D_IT_TE | DMA2D_IT_CE |
                                     DMA2D_IT_CAE);
...
HAL_StatusTypeDef HAL_DMA2D_CLUTLoading_Suspend(DMA2D_HandleTypeDef *hdma2d,
  ...
  loadsuspended =
      ((hdma2d->Instance->CR & DMA2D_CR_SUSP) == DMA2D_CR_SUSP) ? 1UL : 0UL;
  /* 2nd condition: Not Start Check */
  loadsuspended |=
      ((*reg & DMA2D_BGPFCCR_START) != DMA2D_BGPFCCR_START) ? 1UL : 0UL;
...
HAL_StatusTypeDef HAL_DMA2D_CLUTLoading_Resume(DMA2D_HandleTypeDef *hdma2d,
  ...
  if (LayerIdx == DMA2D_BACKGROUND_LAYER) {
    ...
    if ((hdma2d->Instance->CR & DMA2D_CR_SUSP) == DMA2D_CR_SUSP) {
      if ((hdma2d->Instance->BGPFCCR & DMA2D_BGPFCCR_START) ==
          DMA2D_BGPFCCR_START) {
        /* Ongoing CLUT loading is suspended: change the DMA2D state before
         * resuming */
        hdma2d->State = HAL_DMA2D_STATE_BUSY;
      }
    }
  ...
  if (layer_start != 0U) {
    ...
    while (__HAL_DMA2D_GET_FLAG(hdma2d, DMA2D_FLAG_CTC) == 0U) {
      ...
      if ((isrflags & (DMA2D_FLAG_CAE | DMA2D_FLAG_CE | DMA2D_FLAG_TE)) != 0U) {
        ...
        /* Clear the CLUT Access Error, Configuration Error and Transfer Error
         * flags */
        __HAL_DMA2D_CLEAR_FLAG(hdma2d,
                               DMA2D_FLAG_CAE | DMA2D_FLAG_CE | DMA2D_FLAG_TE);
...
__weak void HAL_DMA2D_CLUTLoadingCpltCallback(DMA2D_HandleTypeDef *hdma2d) {
  ...
  /* NOTE : This function should not be modified; when the callback is needed,
            the HAL_DMA2D_CLUTLoadingCpltCallback can be implemented in the user
     file.
   */
}
...
HAL_StatusTypeDef HAL_DMA2D_ConfigLayer(DMA2D_HandleTypeDef *hdma2d,
  ...
  if (hdma2d->Init.Mode != DMA2D_R2M) {
    assert_param(
        IS_DMA2D_INPUT_COLOR_MODE(hdma2d->LayerCfg[LayerIdx].InputColorMode));
    if (hdma2d->Init.Mode != DMA2D_M2M) {
      assert_param(IS_DMA2D_ALPHA_MODE(hdma2d->LayerCfg[LayerIdx].AlphaMode));
    }
  }
  assert_param(
      IS_DMA2D_ALPHA_INVERTED(hdma2d->LayerCfg[LayerIdx].AlphaInverted));
  assert_param(IS_DMA2D_RB_SWAP(hdma2d->LayerCfg[LayerIdx].RedBlueSwap));

  if ((LayerIdx == DMA2D_FOREGROUND_LAYER) &&
      (hdma2d->LayerCfg[LayerIdx].InputColorMode == DMA2D_INPUT_YCBCR)) {
    assert_param(IS_DMA2D_CHROMA_SUB_SAMPLING(
        hdma2d->LayerCfg[LayerIdx].ChromaSubSampling));
  }
  ...
  regValue = pLayerCfg->InputColorMode |
             (pLayerCfg->AlphaMode << DMA2D_BGPFCCR_AM_Pos) |
             (pLayerCfg->AlphaInverted << DMA2D_BGPFCCR_AI_Pos) |
             (pLayerCfg->RedBlueSwap << DMA2D_BGPFCCR_RBS_Pos);
  regMask = (DMA2D_BGPFCCR_CM | DMA2D_BGPFCCR_AM | DMA2D_BGPFCCR_ALPHA |
             DMA2D_BGPFCCR_AI | DMA2D_BGPFCCR_RBS);
...
HAL_StatusTypeDef HAL_DMA2D_ConfigDeadTime(DMA2D_HandleTypeDef *hdma2d,
  ...
  MODIFY_REG(hdma2d->Instance->AMTCR, DMA2D_AMTCR_DT,
             (((uint32_t)DeadTime) << DMA2D_AMTCR_DT_Pos));
...
static void DMA2D_SetConfig(DMA2D_HandleTypeDef *hdma2d, uint32_t pdata,
  ...
  if (hdma2d->Init.Mode == DMA2D_R2M) {
    ...
    /* Prepare the value to be written to the OCOLR register according to the
     * color mode */
  ...
  } else if (hdma2d->Init.Mode ==
             DMA2D_M2M_BLEND_FG) /*M2M_blending with fixed color FG DMA2D Mode
                                    selected*/
  {
    WRITE_REG(hdma2d->Instance->BGMAR, pdata);
  } else /* M2M, M2M_PFC,M2M_Blending or M2M_blending with fixed color BG DMA2D
            Mode */
  {
