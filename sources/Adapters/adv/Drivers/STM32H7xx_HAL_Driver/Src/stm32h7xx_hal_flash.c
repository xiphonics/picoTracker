...
void HAL_FLASH_IRQHandler(void) {
  ...
  if (__HAL_FLASH_GET_FLAG_BANK1(FLASH_SR_EOP) != RESET) {
    if (pFlash.ProcedureOnGoing == FLASH_PROC_SECTERASE_BANK1) {
      ...
      if ((procedure == FLASH_PROC_MASSERASE_BANK1) ||
          (procedure == FLASH_PROC_ALLBANK_MASSERASE)) {
  ...
  if (__HAL_FLASH_GET_FLAG_BANK2(FLASH_SR_EOP) != RESET) {
    if (pFlash.ProcedureOnGoing == FLASH_PROC_SECTERASE_BANK2) {
      ...
      if ((procedure == FLASH_PROC_MASSERASE_BANK2) ||
          (procedure == FLASH_PROC_ALLBANK_MASSERASE)) {
        /*MassErase ended. Return the selected bank*/
        /* FLASH EOP interrupt user callback */
  ...
  errorflag = FLASH->SR1 & (FLASH_FLAG_WRPERR_BANK1 | FLASH_FLAG_PGSERR_BANK1 |
  ...
  if (errorflag != 0U) {
    ...
    if (procedure == FLASH_PROC_SECTERASE_BANK1) {
      ...
      temp = pFlash.Sector;
      pFlash.Sector = 0xFFFFFFFFU;
    } else if ((procedure == FLASH_PROC_MASSERASE_BANK1) ||
               (procedure == FLASH_PROC_ALLBANK_MASSERASE)) {
  ...
  if (errorflag != 0U) {
    ...
    /* Clear FLASH Bank1 ECC single correction error flag in order to allow new
     * ECC error record */
    __HAL_FLASH_CLEAR_FLAG_BANK1(errorflag);
  }
  ...
  if (errorflag != 0U) {
    ...
    /* Clear FLASH Bank1 ECC double detection error flag in order to allow new
     * ECC error record */
    __HAL_FLASH_CLEAR_FLAG_BANK1(errorflag);
  }
#endif /* USE_FLASH_ECC */
...
#if defined(FLASH_SR_OPERR)
  errorflag = FLASH->SR2 & ((FLASH_FLAG_WRPERR_BANK2 | FLASH_FLAG_PGSERR_BANK2 |
                             FLASH_FLAG_STRBERR_BANK2 |
                             FLASH_FLAG_INCERR_BANK2 | FLASH_FLAG_OPERR_BANK2) &
                            0x7FFFFFFFU);
#else
  errorflag =
      FLASH->SR2 & ((FLASH_FLAG_WRPERR_BANK2 | FLASH_FLAG_PGSERR_BANK2 |
  ...
  if (errorflag != 0U) {
    ...
    if (procedure == FLASH_PROC_SECTERASE_BANK2) {
      ...
      temp = pFlash.Sector;
      pFlash.Sector = 0xFFFFFFFFU;
    } else if ((procedure == FLASH_PROC_MASSERASE_BANK2) ||
               (procedure == FLASH_PROC_ALLBANK_MASSERASE)) {
      /*return the faulty bank*/
      temp = FLASH_BANK_2;
    } else {
  ...
  if (errorflag != 0U) {
    ...
    /* Clear FLASH Bank2 ECC single correction error flag in order to allow new
     * ECC error record */
    __HAL_FLASH_CLEAR_FLAG_BANK2(errorflag);
  }
  ...
  if (errorflag != 0U) {
    ...
    /* Clear FLASH Bank2 ECC double detection error flag in order to allow new
     * ECC error record */
    __HAL_FLASH_CLEAR_FLAG_BANK2(errorflag);
  }
...
HAL_StatusTypeDef HAL_FLASH_OB_Launch(void) {
  ...
  if (FLASH_CRC_WaitForLastOperation((uint32_t)FLASH_TIMEOUT_VALUE,
                                     FLASH_BANK_1) != HAL_OK) {
    status = HAL_ERROR;
  }
#if defined(DUAL_BANK)
  else if (FLASH_CRC_WaitForLastOperation((uint32_t)FLASH_TIMEOUT_VALUE,
                                          FLASH_BANK_2) != HAL_OK) {
    status = HAL_ERROR;
  }
#endif /* DUAL_BANK */
 ...
 *            @arg HAL_FLASH_ERROR_SNECC_BANK1: ECC Single Correction Error on
 * Bank 1
 *            @arg HAL_FLASH_ERROR_DBECC_BANK1: ECC Double Detection Error on
 * Bank 1
...
HAL_StatusTypeDef FLASH_OB_WaitForLastOperation(uint32_t Timeout) {
  ...
  /* Wait for the FLASH Option Bytes change operation to complete by polling on
   * OPT_BUSY flag to be reset */
...
HAL_StatusTypeDef FLASH_CRC_WaitForLastOperation(uint32_t Timeout,
  ...
  /* Wait for the FLASH CRC computation to complete by polling on CRC_BUSY flag
   * to be reset */
  while (__HAL_FLASH_GET_FLAG(bsyflag)) {
    if (Timeout != HAL_MAX_DELAY) {
      if (((HAL_GetTick() - tickstart) > Timeout) || (Timeout == 0U)) {
