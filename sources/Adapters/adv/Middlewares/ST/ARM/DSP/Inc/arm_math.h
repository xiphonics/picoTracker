...
/**
   ...
   * a suite of common signal processing functions for use on Cortex-M and
   Cortex-A processor
   * based devices.
   *
   ...
   * - Distance functions
   *
   * The library has generally separate functions for operating on 8-bit
   ...
   *
   * There is also a work in progress cmake build. The README file is giving
   more details.
   *
   * Preprocessor Macros
   * ------------
...
/**
 * @brief 16-bit fractional 128-bit vector data type with 16-bit alignement
 * in 1.15 format.
 */
typedef __ALIGNED(2) int16x8_t q15x8_t;

/**
 * @brief 8-bit fractional 128-bit vector data type with 8-bit alignement in 1.7
 * format.
 */
...
#if defined(ARM_MATH_NEON) || defined(ARM_MATH_MVEF) /* floating point         \
                                                        vector*/
/**
 ...
 */
__STATIC_INLINE void arm_norm_64_to_32u(uint64_t in, int32_t *normalized,
                                        int32_t *norm) {
  int32_t n1;
...
/**
 * @brief Instance structure for the modified Biquad coefs required by
 * vectorized code.
 */
typedef struct {
  float32_t coeffs[8][4]; /**< Points to the array of modified coefficients. The
                             array is of length 32. There is one per stage */
 ...
 * @param[in]     numStages  number of 2nd order stages in the filter.
 * @param[in]     pCoeffs    points to the filter coefficients.
 * @param[in]     pCoeffsMod points to the modified filter coefficients (only
 * MVE version).
 * @param[in]     pState     points to the state buffer.
 */
#if defined(ARM_MATH_MVEF) && !defined(ARM_MATH_AUTOVECTORIZE)
 ...
 * @return        none
 */
void arm_and_u16(const uint16_t *pSrcA, const uint16_t *pSrcB, uint16_t *pDst,
                 uint32_t blockSize);
 ...
 * @return        none
 */
void arm_and_u32(const uint32_t *pSrcA, const uint32_t *pSrcB, uint32_t *pDst,
                 uint32_t blockSize);
 ...
 * @return        none
 */
void arm_and_u8(const uint8_t *pSrcA, const uint8_t *pSrcB, uint8_t *pDst,
                uint32_t blockSize);
 ...
 * @return        none
 */
void arm_or_u16(const uint16_t *pSrcA, const uint16_t *pSrcB, uint16_t *pDst,
                uint32_t blockSize);
 ...
 * @return        none
 */
void arm_or_u32(const uint32_t *pSrcA, const uint32_t *pSrcB, uint32_t *pDst,
                uint32_t blockSize);
 ...
 * @return        none
 */
void arm_or_u8(const uint8_t *pSrcA, const uint8_t *pSrcB, uint8_t *pDst,
               uint32_t blockSize);
 ...
 * @return        none
 */
void arm_xor_u16(const uint16_t *pSrcA, const uint16_t *pSrcB, uint16_t *pDst,
                 uint32_t blockSize);
 ...
 * @return        none
 */
void arm_xor_u32(const uint32_t *pSrcA, const uint32_t *pSrcB, uint32_t *pDst,
                 uint32_t blockSize);
 ...
 * @return        none
 */
void arm_xor_u8(const uint8_t *pSrcA, const uint8_t *pSrcB, uint8_t *pDst,
                uint32_t blockSize);
 ...
 * @param[in]  blockSize  number of samples to process.
 */
void arm_sort_f32(const arm_sort_instance_f32 *S, float32_t *pSrc,
                  float32_t *pDst, uint32_t blockSize);
 ...
 * @param[in]      blockSize  number of samples to process.
 */
void arm_merge_sort_f32(const arm_merge_sort_instance_f32 *S, float32_t *pSrc,
                        float32_t *pDst, uint32_t blockSize);
 ...
 */
void arm_merge_sort_init_f32(arm_merge_sort_instance_f32 *S, arm_sort_dir dir,
                             float32_t *buffer);
 ...
 * @param[in]  S          points to an instance of the floating-point spline
 * structure.
 * @param[in]  xq         points to the x values ot the interpolated data
 * points.
 * @param[out] pDst       points to the block of output data.
 * @param[in]  blockSize  number of samples of output data.
 */
void arm_spline_f32(arm_spline_instance_f32 *S, const float32_t *xq,
                    float32_t *pDst, uint32_t blockSize);

/**
 * @brief Initialization function for the floating-point cubic spline
 * interpolation.
 * @param[in,out] S        points to an instance of the floating-point spline
 * structure.
 * @param[in]     type     type of cubic spline interpolation (boundary
 * conditions)
 ...
 */
void arm_spline_init_f32(arm_spline_instance_f32 *S, arm_spline_type type,
                         const float32_t *x, const float32_t *y, uint32_t n,
...
void arm_rfft_fast_f64(arm_rfft_fast_instance_f64 *S, float64_t *p,
                       float64_t *pOut, uint8_t ifftFlag);
...
typedef struct {
  ...
  float64_t *pState; /**< points to the array of state coefficients.  The array
                        is of length 2*numStages. */
  const float64_t *pCoeffs; /**< points to the array of coefficients.  The array
                               is of length 5*numStages. */
...
void arm_biquad_cascade_df2T_f64(const arm_biquad_cascade_df2T_instance_f64 *S,
                                 const float64_t *pSrc, float64_t *pDst,
                                 uint32_t blockSize);
...
/**
  ...
  @return        none
 */
void arm_max_no_idx_f32(const float32_t *pSrc, uint32_t blockSize,
                        float32_t *pResult);
 ...
 * @param[in]    S                      points to an instance of the polynomial
 * SVM structure.
 * @param[in]    nbOfSupportVectors     Number of support vectors
 * @param[in]    vectorDimension        Dimension of vector space
 * @param[in]    intercept              Intercept
...
void arm_svm_polynomial_init_f32(arm_svm_polynomial_instance_f32 *S,
                                 ...
                                 const float32_t *dualCoefficients,
                                 const float32_t *supportVectors,
                                 const int32_t *classes, int32_t degree,
                                 float32_t coef0, float32_t gamma);

/**
 * @brief SVM polynomial prediction
 * @param[in]    S          Pointer to an instance of the polynomial SVM
 * structure.
 * @param[in]    in         Pointer to input vector
 * @param[out]   pResult    Decision value
 * @return none.
 ...
 * @param[in]    S                      points to an instance of the rbf SVM
 * structure.
 * @param[in]    nbOfSupportVectors     Number of support vectors
 * @param[in]    vectorDimension        Dimension of vector space
 * @param[in]    intercept              Intercept
...
void arm_svm_sigmoid_init_f32(arm_svm_sigmoid_instance_f32 *S,
                              ...
                              const float32_t *dualCoefficients,
                              const float32_t *supportVectors,
                              const int32_t *classes, float32_t coef0,
                              float32_t gamma);
 ...
 *
 * In probabilistic computations, the dynamic of the probability values can be
 * very wide because they come from gaussian functions. To avoid underflow and
 * overflow issues, the values are represented by their log. In this
 * representation, multiplying the original exp values is easy : their logs are
 * added. But adding the original exp values is requiring some special handling
 * and it is the goal of the LogSumExp function.
 *
...
float32_t arm_logsumexp_dot_prod_f32(const float32_t *pSrcA,
                                     const float32_t *pSrcB, uint32_t blockSize,
 ...
 *
 * @param[in]  pSrcA         Pointer to an array of input values for probability
 * distribution A.
 * @param[in]  pSrcB         Pointer to an array of input values for probability
 * distribution B.
 * @param[in]  blockSize     Number of samples in the input array.
...
float64_t arm_kullback_leibler_f64(const float64_t *pSrcA,
                                   const float64_t *pSrcB, uint32_t blockSize);
 ...
 *
 */
float32_t arm_weighted_sum_f32(const float32_t *in, const float32_t *weigths,
                               uint32_t blockSize);
 ...
 *
 */
void arm_barycenter_f32(const float32_t *in, const float32_t *weights,
                        float32_t *out, uint32_t nbVectors, uint32_t vecDim);
...
float32_t arm_euclidean_distance_f32(const float32_t *pA, const float32_t *pB,
                                     uint32_t blockSize);
 ...
 * The result of the computation will be correct. So the division per zero may
 * be ignored.
 *
 * @param[in]    pA         First vector
 * @param[in]    pB         Second vector
 ...
 *
 */
float32_t arm_canberra_distance_f32(const float32_t *pA, const float32_t *pB,
                                    uint32_t blockSize);
 ...
 *
 */
float32_t arm_chebyshev_distance_f32(const float32_t *pA, const float32_t *pB,
                                     uint32_t blockSize);
 ...
 *
 */
float32_t arm_cityblock_distance_f32(const float32_t *pA, const float32_t *pB,
                                     uint32_t blockSize);
 ...
 *
 */
float32_t arm_correlation_distance_f32(float32_t *pA, float32_t *pB,
                                       uint32_t blockSize);
...
float32_t arm_cosine_distance_f32(const float32_t *pA, const float32_t *pB,
                                  uint32_t blockSize);
...
float32_t arm_minkowski_distance_f32(const float32_t *pA, const float32_t *pB,
                                     int32_t order, uint32_t blockSize);
...
float32_t arm_dice_distance(const uint32_t *pA, const uint32_t *pB,
...
float32_t arm_hamming_distance(const uint32_t *pA, const uint32_t *pB,
                               uint32_t numberOfBools);
...
float32_t arm_jaccard_distance(const uint32_t *pA, const uint32_t *pB,
                               uint32_t numberOfBools);
...
float32_t arm_kulsinski_distance(const uint32_t *pA, const uint32_t *pB,
                                 uint32_t numberOfBools);
...
float32_t arm_russellrao_distance(const uint32_t *pA, const uint32_t *pB,
                                  uint32_t numberOfBools);
...
float32_t arm_sokalsneath_distance(const uint32_t *pA, const uint32_t *pB,
                                   uint32_t numberOfBools);
...
float32_t arm_yule_distance(const uint32_t *pA, const uint32_t *pB,
                            uint32_t numberOfBools);
...
__STATIC_FORCEINLINE float32_t arm_bilinear_interp_f32(
  ...
  if (xIndex < 0 || xIndex > (S->numCols - 2) || yIndex < 0 ||
      yIndex > (S->numRows - 2)) {
    return (0);
  }
