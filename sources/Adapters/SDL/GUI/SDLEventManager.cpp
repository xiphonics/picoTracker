...
bool SDLEventManager::Init() {
  ...
  for (int i = 0; i < joyCount; i++) {
    ...
    Trace::Log("EVENT", "Number of buttons:%d",
               SDL_JoystickNumButtons(joystick_[i]));
  ...
  while (!finished_) {
    ...
    if (SDL_WaitEvent(&event)) {
      ...
      case SDL_JOYBUTTONUP:
        if (dumpEvent_) {
          Trace::Log("EVENT", "but(%d):%d", event.button.which,
      ...
      case SDL_JOYAXISMOTION:
        if (dumpEvent_) {
          Trace::Log("EVENT", "joy(%d)::%d=%d", event.jaxis.which,
                     event.jaxis.axis, event.jaxis.value);
        }
        joystickCS_[event.jaxis.which]->SetAxis(
            event.jaxis.axis, float(event.jaxis.value) / 32767.0f);
        break;
        ...
        if (dumpEvent_) {
          for (int i = 0; i < 4; i++) {
            ...
            if (event.jhat.value & mask) {
              Trace::Log("EVENT", "hat(%d)::%d::%d", event.jhat.which,
                         event.jhat.hat, i);
            }
          }
        }
      ...
      case SDL_JOYBALLMOTION:
        if (dumpEvent_) {
          Trace::Log("EVENT", "ball(%d)::%d=(%d,%d)", event.jball.which,
