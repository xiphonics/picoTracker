...
UnixFile::UnixFile(FILE *file) { file_ = file; };
...
void UnixFile::Seek(long offset, int whence) { fseek(file_, offset, whence); };

long UnixFile::Tell() { return ftell(file_); };
void UnixFile::Close() {
  fflush(file_);
  fsync(fileno(file_));
  fclose(file_);
...
I_Dir *UnixFileSystem::Open(const char *path) { return new UnixDir(path); };
...
void UnixFileSystem::Delete(const char *path) { remove(path); };
