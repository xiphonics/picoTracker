...
static void draw_button(uint8_t x, uint8_t y, char label, bool invert) {
  const char line[4] = {char_border_single_vertical, label, char_border_single_vertical, 0};
  draw_string(x, y + 0, char_border_single_topLeft_s char_border_single_horizontal_s char_border_single_topRight_s, invert);
  draw_string(x, y + 1, (const char *)line, invert);
  draw_string(x, y + 2, char_border_single_bottomLeft_s char_border_single_horizontal_s char_border_single_bottomRight_s, invert);
}
...
void drawInputTester() {
  ...
  // Draw button tester at top of screen
  // Layout:
  //   UPS
  //  LDRE
  //   AN
