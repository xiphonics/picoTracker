...
While all releases are built on CI using GitHub Actions on the release branch, some manual steps are still required to create a new release:

1. Update the build number (`PROJECT_NUMBER` define) in `sources/Application/Model/Project.h`
1. Update the version in `sources/Adapters/adv/Core/Src/main.c`
1. Create a PR to merge into the release branch and have it merged
